...
- [x] Crear `Resources/Shared.es-ES.resx`, `Resources/Shared.en-US.resx` (Commit 1e91cda)
- [ ] Extraer literales de componentes base (botones, estados) (Parcial: NavMenu + Employees)
- [ ] Middleware cultura (por TenantContext.Culture) (Base: RequestLocalization configurado; falta integración Tenant)

Observabilidad
- [ ] Servicio `UiTelemetry` + Application Insights (TrackEvent) (servicio stub presente, falta instrumentación)
...
Verificación contra código real (última actualización incluyendo commits Polly + i18n inicial):
...
- Nuevo: Retry/backoff (Polly) agregado a HttpClient del ApiAdapter (`ServiceCollectionExtensions.cs`, commit 0ae1c66).
- Nuevo: Internacionalización inicial (recursos `Shared.es-ES/en-US` + uso en NavMenu y Employees, commit 1e91cda).
- Parcial: Localización solo aplicada a una página y menú (faltan componentes base).
- Pendientes: gating licencia, patrón descargas, telemetría eventos, auditoría UI, selector interactivo de tenant, accesibilidad AA refinada.
- DataGrid externo: decisión pendiente (mantener custom vs adoptar librería para virtual scroll/filtros avanzados).
- Próximo enfoque P0 sugerido: instrumentación mínima (registrar view.opened y data.load.*), scaffolding patrón descargas (`DownloadClient` + `AppDownloadButton`) y avance i18n (extraer botones comunes + estados).

...
if ( $args.count -eq 3 ){
    ...
    $onlySlots=$args[2].ToLower()

    if(!($onlySlots -eq "false")){ $onlySlots ='true'}
} else{
    Write-Output "Atributtes missing: correct usage 'newAppFunction.ps1 functionName=fnxxxxxxxxxxxxxxxxxx slots=idi0x' onlyslots=true"
    exit
}


Write-Host ("Se van a crear la siguientes funciones(" + $functionsName + ")")
Write-Host ("... Creación de slots:" + $publishSlots)
Write-Host ("... Omitir creación de función:" + $onlySlots)
...
if( $publishSlots )
{
	...
	Foreach ($functionName in $functionsNameArray)
    {
	    ...
	    Foreach ($publishSlot in $publishSlotsArray)
        {
            ...
            if( !($publishSlot -eq "empty"))
            {
		        Write-Host ("... ... Creando slot:" + $publishSlot) 
		        ...
		        az functionapp deployment slot create --name $functionName --resource-group mtdev --slot $publishSlot | out-null
                Write-Host ("... ... ... OK") 
            
                Write-Host ("... ... Configurando slot:" + $publishSlot) 
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "DebugMode=true" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "VTLive.MultitenantService=true" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "WEBSITE_TIME_ZONE=Romance Standard Time" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings '"CosmosDB.ConnectionString='$cosmosKey'"' --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "CosmosDB.DBName=$publishSlot" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "ApplicationInsights.Key=$appInsightsSlotKey" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "AzureWebJobsServiceBus=$serviceBusSlotKey" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "AzureWebJobsDashboard=$storageSlotKey" --slot $publishSlot | out-null
                az functionapp config appsettings set --name $functionName --resource-group mtdev --slot-settings "AzureWebJobsStorage=$storageSlotKey" --slot $publishSlot | out-null
                Write-Host ("... ... ... OK") 

                az functionapp stop --name $functionName --resource-group mtdev --slot $publishSlot
            }

...
$filepath = Get-Item -Path ".\..\..\..\..\..\VTPortalJS\deploy iOS\config.xml"
$sourcePath = [System.IO.Path]::GetDirectoryName($filepath.FullName)
$destinationPath = ('c:\temp\vtportal.ios.c' + $versionCode + '.v' + $version + '.zip')

Write-Host ("... ... Generando zip iOS en " + $destinationPath)
[System.IO.Compression.ZipFile]::CreateFromDirectory("$sourcePath","$destinationPath",$compressionLevel,$includeBaseDirectory)

uploadToVoltBuild $destinationPath "ios"

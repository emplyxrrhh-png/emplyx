...
using Robotics.Base.VTPunchConnector;
...
namespace roPunchConnectorFunctions
{
    public class RunConnector
    {
        ...
        public RunConnector(ILoggerFactory loggerFactory)
        {
            _logger = loggerFactory.CreateLogger<RunConnector>();
        }

        [Function("RunConnector")]
        public void Run([ServiceBusTrigger("connectortasks", Connection = "AzureWebJobsServiceBus")] string myQueueItem, ILogger log)
        {
            Robotics.Azure.RoAzureSupport.SetDefaultLogLevel(Program.DefaultLogLevel, Program.DefaultTraceLevel, "roPunchConnectorFunction");
            ...
            try
            {
                ...
                if (strTaskId != "-999")
                {
                    ...
                    {
                        roLog.get_GetInstance().logMessage(Robotics.VTBase.roLog.EventType.roError, "RunConnector::Could not retrieve company information::" + strCompany);
                    }
                    else
                    {
            ...
            {
                roLog.get_GetInstance().logMessage(Robotics.VTBase.roLog.EventType.roError, "RunConnector::" + strTaskId + "::Fatal error::Exception::" + e.Message);
            }
        }
        ...
        private static async Task<int> ExecuteCompanyAction(roCompanyConfiguration oCompanyConf, int taskId, int defaultLogLevel, int defaultTraceLevel)
        {
            return await Task.Run(() =>
            {
                ...
                {
                    bool bInit = Robotics.DataLayer.AccessHelper.SetThreadInformation("roPunchConnectorFunction", oCompanyConf, defaultLogLevel, defaultTraceLevel,taskId);

                    if (bInit)
                    {
                        var oTask = new ConnectorTask();

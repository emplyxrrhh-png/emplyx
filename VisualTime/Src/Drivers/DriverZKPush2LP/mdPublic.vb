...
Public Module mdPublic
    ...
    Public gLog As New roLog("DriverZKPush2")
    ...
    Public Function NormalizeText(sText As String) As String
        Try
            Dim sNormText As String
            sNormText = sText.Normalize(System.Text.NormalizationForm.FormD)
            Dim reg As New System.Text.RegularExpressions.Regex("[^a-zA-Z0-9 ]")
            Return reg.Replace(sNormText, "")
        Catch ex As Exception
            Return sText
        End Try
    End Function

    Public Function AnonimizeLog(sText As String) As String
        Dim retText As String = sText
        Try
            ' Clave de comunicaciones
            If retText.Contains("COMKey") Then
                Dim sComkeyText As String = String.Empty
                sComkeyText = retText.Substring(retText.IndexOf("COMKey"), 13)
                retText = retText.Replace(sComkeyText, "COMKey=?")
            End If
            ' Usuario de administración
            If retText.Contains("PIN=9999") Then
                Dim sPwdText As String = String.Empty
                Dim sAux As String = String.Empty
                sAux = retText.Substring(retText.IndexOf("Passwd="))
                sPwdText = sAux.Substring(0, sAux.IndexOf(vbTab))
                retText = retText.Replace(sPwdText, "Passwd=?")
            End If
        Catch ex As Exception
            Return sText
        End Try
    ...
    Public Sub CreateUserTask(ByVal ID As Integer)
        ...
        Dim oTaskExist As New VTBusiness.UserTask.roUserTask("USERTASK:\\TERMINAL_TYPEMISMATCH_" & ID.ToString, oState)
    ...
    Public Function DecodeRoboticsCard(ByVal IDCard As String, Optional ByVal MaxLen As Byte = 16) As String
        ...
        Try
            ...
            If IDCard.Trim <> "" Then
                ...
                Catch ex As Exception
                    sIDCard = BigInteger.Parse(sIDCard, System.Globalization.NumberStyles.HexNumber).ToString
                End Try
            End If

            Return sIDCard.TrimStart("0"c)
        Catch ex As Exception
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function ConvertCardForTerminal(_VTCard As String) As String
        Try
    ...
    Public Function ConvertCardForDatabase(_VTcard As String) As String
        Try
            Select Case clsParameters.CardType
                Case clsParameters.eCardType.HID
                    ' Tarjeta HID.
                    ...
                    'Return _Card
                    Return mdPublic.AddHIDParityBits(_VTcard)
                Case clsParameters.eCardType.Mifare
                    ' Tarjeta MiFare. En BBDD es guarda con codificación Robotics
                    Return mdPublic.EncodeRoboticsCard(_VTcard)
                Case clsParameters.eCardType.Unique
                    ' Tarjeta UNIQUE, con codificación Robotics
                    Return mdPublic.EncodeRoboticsCard(_VTcard)
                Case clsParameters.eCardType.UniqueNumeric
            ...
            End Select
        Catch ex As Exception
            gLog.logMessage(roLog.EventType.roError, "mdPublic::ConvertCardForDatabase::Error:(IDCard:" + _VTcard + "):", ex)
            Return _VTcard
        End Try
    End Function

    Public Function ConvertCardForTerminal(_VTCard As String, _CardType As clsParameters.eCardType, Optional mUniqueReadBytes As Integer = 3) As String
        Try
            Select Case _CardType
                ...
                Case clsParameters.eCardType.Mifare
                    ' Tarjeta MiFare. En BBDD es guarda con codificación Robotics
                    If _VTCard.Length > 28 Then
                        ' Soporte para tarjetas PassVigo - (MiFare Plus 10 bytes) 
                        Return mdPublic.DecodeRoboticsCard(_VTCard, 100)
                    Else
                        Return mdPublic.DecodeRoboticsCard(_VTCard, 16)
                    End If
                Case clsParameters.eCardType.Unique
                    ' Tarjeta UNIQUE, con codificación Robotics, para rxte son 4 bytes (16) y para el resto 3 (12)
                    Return mdPublic.DecodeRoboticsCard(_VTCard, mUniqueReadBytes * 4)
                Case clsParameters.eCardType.UniqueNumeric
                    ' Tarjeta UNIQUE, sin codificación Robotics. Simplemente corto a 3 bytes (6 caracteres HEXA) o 4 bytes (8 caracteres HEXA)
                    Return CutCard(_VTCard, mUniqueReadBytes * 2)
                Case clsParameters.eCardType.MifareNumeric
                    Return CutCard(_VTCard, 8)
                Case Else
                    Return CutCard(_VTCard, 6)
    ...
    Public Function ConvertCardForDatabase(_VTcard As String, _CardType As clsParameters.eCardType) As String
        Try
            Select Case _CardType
                Case clsParameters.eCardType.HID
                    ' Tarjeta HID.
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function EncodeRoboticsCard(ByVal IDCard As String) As String
        Dim sIDCard As String = ""
        Dim tmp As String = ""
        Try
            ' Pasamos a HEXA
            ...
            Catch ex As Exception
                'Protección sobre tarjetas mifare de 10 bytes por ejemplo PassVigo
                tmp = BigInteger.Parse(IDCard).ToString("X")
            End Try
    ...
    Public Function GetZKPUSHTerminalsRequestDelay() As Integer
        Try
            Dim sLiveAdvancedParameter As String = String.Empty
            clsParameters.LoadAdvancedParameter("ZKPUSHTerminalsRequestDelay", sLiveAdvancedParameter)
            Dim delay As Integer = roTypes.Any2Integer(sLiveAdvancedParameter)
            If delay < 150 Then delay = 500
            Return delay
        Catch ex As Exception
            Return 500
        End Try
    ...
    Public Function GetZKPUSHTerminalTimeZoneName(ByVal SN As String) As String
        Try
            Dim sql As String = String.Empty
            sql = $"@SELECT# Timezonename FROM Terminals WHERE SerialNumber = '{SN}'"
            Return roTypes.Any2String(AccessHelper.ExecuteScalar(sql))
        Catch ex As Exception

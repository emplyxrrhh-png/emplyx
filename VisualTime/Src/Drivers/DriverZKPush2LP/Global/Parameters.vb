...
Public Class clsParameters
    ...
    Public Shared ReadOnly Property DefaultLanguage() As String
        Get
            Load()
            Return oSettings.GetVTSetting(eKeys.DefaultLanguage)
        End Get
    End Property
    ...
    ' Tiempo mínimo entre una entrada y una salida. En minutos
    Public Shared ReadOnly Property PunchPeriodRTIn() As Integer
        Get
            Load()

            ' Primero miramos si está definido el parámetro en la tabla sysroParameters
            If oParameters.Parameter(Parameters.PunchPeriodRTIn) IsNot Nothing Then
                Return roTypes.Any2Integer(oParameters.Parameter(Parameters.PunchPeriodRTIn))
            Else
                ' Sino lo obtenemos del registro
                Dim intRet As Integer = oSettings.GetVTSetting(eKeys.PunchPeriodRTIn)
                If intRet = 0 Then intRet = 2
                Return intRet
            End If
        End Get
    End Property

    ' Tiempo mínimo entre una salida y una entrada. En minutos
    Public Shared ReadOnly Property PunchPeriodRTOut() As Integer
        Get

            Load()

            ' Primero miramos si está definido el parámetro en la tabla sysroParameters
            If oParameters.Parameter(Parameters.PunchPeriodRTOut) IsNot Nothing Then
                Return roTypes.Any2Integer(oParameters.Parameter(Parameters.PunchPeriodRTOut))
            Else
                ' Sino lo obtenemos del registro
                Dim intRet As Integer = oSettings.GetVTSetting(eKeys.PunchPeriodRTOut)
                If intRet = 0 Then intRet = 3
                Return intRet
            End If
        End Get
    End Property

    Public Shared ReadOnly Property JobTimePermisionType() As Integer
        Get
            Load()

            Return _JobTimePermissionType
        End Get
    End Property

    Public Shared ReadOnly Property EmployeesJobTimePermission() As String
        Get
            Load()

            Return _EmployeesJobTimePermission
        End Get
    End Property

    Public Shared ReadOnly Property OpenJobMoveOnEntri() As Boolean
        Get
            Load()

            Return roTypes.Any2Boolean(oSettings.GetVTSetting(eKeys.AutomaticBeginJob))
        End Get
    End Property

    ''' <summary>
    ''' Tiempo máximo entre una entrada y una salida. En minutos
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared ReadOnly Property MovMaxHours() As Integer
        Get
            Load()

            ' Primero miramos si está definido el parámetro en la tabla sysroParameters
            If oParameters.Parameter(Parameters.MovMaxHours) IsNot Nothing Then
                Return roTypes.Any2Time(oParameters.Parameter(Parameters.MovMaxHours)).Hours
            Else
                Return 0
            End If
        End Get
    ...
    Public Shared ReadOnly Property PrinterMarginLeft() As Integer
        Get
            Dim val As Integer = 0
            val = roTypes.Any2Integer(getXMLParams(eXMLParams.PrinterMarginLeft))
            Return val
        End Get
    End Property

    Public Shared ReadOnly Property PrinterMarginTop() As Integer
        Get
            Dim val As Integer = 0
            val = roTypes.Any2Integer(getXMLParams(eXMLParams.PrinterMarginTop))
            Return val
        ...
        Get
            If _CardCode = eCardCode.None Then
                Try
                    ...
                    sValue = roTypes.Any2Integer(Microsoft.Win32.Registry.GetValue(_RegistryRoot + "Robotics\VisualTime\Process\CommsNET", "CardType", 2))
                    Select Case sValue
                        Case "0", "Hex"
    ...
    Public Shared Sub ParseXMLParams(ByVal strParams As String, ByRef mCardtype As eCardType)
        Try
            Dim key As String = ""
            Dim val As String = ""
    ...
    Public Shared Function LoadAdvancedParameter(ByVal strParameterName As String, ByRef strParameterValue As String) As Boolean
        ...
        Catch ex As Data.Common.DbException
            gLog.logMessage(VTBase.roLog.EventType.roError, "TerminalMX7::LoadAdvancedParameter::Load::Error:", ex)
        Catch ex As Exception
            gLog.logMessage(VTBase.roLog.EventType.roError, "TerminalMX7::LoadAdvancedParameter::Load::Error:", ex)
        Finally

...
function BackToGrid() {
    ...
    $("#readOnlyNameDocumentTemplate").html($("#hdnCaptionGrid").val());
...
function AutoEditDocumentTemplate(idDocumentTemplate) {
    if (idDocumentTemplate && idDocumentTemplate != '' && idDocumentTemplate != '-1') {
        
        if (idDocumentTemplate == "0") {
            $('.btnTbAddTaskTemplate').click();
        } else {
            $('#' + GridDocumentTemplate.name).hide();
            $('#divTabInfo').hide();
            $('#tdBackToGrid').hide();
            var tmp = GridDocumentTemplate.ConvertKeyToVisibleIndex(idDocumentTemplate);
            GridDocumentTemplate.RaiseItemDblClick(tmp, document.getElementById(GridDocumentTemplate.GetRow(tmp).id));
        }
    }
}

<%@ Control Language="VB" AutoEventWireup="false" Inherits="VTLive40.Base_WebUserControls_roTreeV3b" CodeBehind="roTreeV3b.ascx.vb" %>
        ...
        <div style="text-align: center; width: 100%; height: 100%">
            ...
            <div style="width: 100%; align-content: center; clear: both; position: relative; top: 80px;">
                <a href="javascript: void(0)" style="cursor: pointer" onclick="<%= Me.ClientID %>_addChildren();" title="<%= Me.Language.Translate("Button.AddChildren","roTreeV3") %>">
                    <img alt="" src="<%= Me.Page.ResolveURL("~/Base/Images/tree_selectChildren.png") %>" style="border: 1px solid #C0C0C0; padding: 5px;" />
                </a>
            </div>
            ...
            <div style="width: 100%; align-content: center; clear: both; position: relative; top: 120px;" id="<%= Me.CookieNameTreeV25 %>_selectAllTree">
                <a href="javascript: void(0)" style="cursor: pointer" onclick="<%= Me.ClientID %>_addAll();" title="<%= Me.Language.Translate("Button.AddAll","roTreeV3") %>">
                    <img alt="" src="<%= Me.Page.ResolveURL("~/Base/Images/tree_selectAll.png") %>" style="border: 1px solid #C0C0C0; padding: 5px;" />
                </a>
            </div>
...
<script language="javascript" type="text/javascript">
    ...
    async function <%= Me.ClientID %>_newItemInGrid(sId) {
        try {
            ...
            var path = '<%= Me.Page.ResolveURL("~/Base/WebUserControls/roTreeV3srv.aspx")%>' + "?action=newItemInGrid&ID=" + sId + "&rows=" + arrRows + "&feature=<%= Me.FeatureAlias %>" +
                "&filter=" + sFilter + "&filterUser=" + sFilterUser + ""
    ...
    async function <%= Me.ClientID %>_newChildrenInGrid(sId) {
        try {
            ...
            var path = '<%= Me.Page.ResolveURL("~/Base/WebUserControls/roTreeV3srv.aspx")%>' + "?action=newChildrenInGrid&ID=" + sId + "&rows=" + arrRows + "&feature=<%= Me.FeatureAlias %>" +
                "&filter=" + sFilter + "&filterUser=" + sFilterUser
            AsyncCall("POST", path, "json", "arrGrid", "<%= Me.ClientID %>_loadGrid(arrGrid);");

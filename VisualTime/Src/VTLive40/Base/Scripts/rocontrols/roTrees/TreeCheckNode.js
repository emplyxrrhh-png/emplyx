...
Ext.tree.TreePanel.prototype.getCheckedNodes = function(node) {
    var checked = [], i;
    if (typeof node == 'undefined') {
        //node = this.rootVisible ? this.getRootNode() : this.getRootNode().firstChild;
        ...
        node = this.getRootNode();
        for (i = 0; i < node.childNodes.length; i++) {
            checked = checked.concat(this.getCheckedNodes(node.childNodes[i]));
        }
 ...
 * @return {Array} array of ids of checked nodes
 */
Ext.tree.TreePanel.prototype.getChecked2 = function(node){
	var checked = [], i;
	if( typeof node == 'undefined' ) {
		node = this.rootVisible ? this.getRootNode() : this.getRootNode().firstChild;
	}

	if( node.attributes.checked2 ) {
		checked.push(node.id);
		if( !node.isLeaf() ) {
			for( i = 0; i < node.childNodes.length; i++ ) {
				checked = checked.concat( this.getChecked2(node.childNodes[i]) );
			}
		}
	}
	return checked;
...
Ext.extend(Ext.tree.CheckboxNodeUI, Ext.tree.TreeNodeUI, {
	...
	check : function(state, descend, bulk) {
		...
		// do we have parents?
		//if( parentNode !== null && state ) { **HB**
		if( parentNode !== null && !state ) {
			// if we're checking the box, check it all the way up
			if( parentNode.getUI().check ) {
				//parentNode.getUI().check(state, false, true); **HB**
				parentNode.getUI().check(state, false, false);
			}
		}
		if( descend && !n.isLeaf() ) {
			var cs = n.childNodes;
		...
		if (n.isLeaf) {
		    if (!state) {
		        n.attributes.checked2 = true;
		        if( parentNode !== null ) {
		            if ( !parentNode.isFirst() )
				        parentNode.getUI().check2(true);
		        }		        		        
		    }
		    else {
		        n.attributes.checked2 = false;
		        if( parentNode !== null ) {		            
		            var cs = parentNode.getUI().childNodes;			        
		            if ( cs == null ) {
		                if ( !parentNode.isFirst() )
		                    parentNode.getUI().check2(true);
		            }
	...
	check2 : function(state) {
		...
		n.attributes.checked2 = state;
       		
		if( parentNode !== null ) {			
			if ( !parentNode.isFirst() )
			    parentNode.getUI().check2(state);
		}

<%@ WebHandler Language="VB" Class="srvBusinessCenters" %>
...
Public Class srvBusinessCenters
    ...
    Public Overrides Sub ProcessRoboticsRequest(ByVal context As HttpContext)
        ...
        If Me.oPermission > Permission.None Then
            ...
            Select Case context.Request("action")
                Case "getBusinessCentersTab" ' Retorna la capcelera de la plana
                    LoadBusinessCentersDataTab()
                Case "deleteXBusinessCenter" ' Elimina un BusinessCenters (JSON)
                    DeleteBusinessCenterDataX(Request("ID"))
                Case "getBarButtons"
...
#Region "Methods"
    ...
    Private Sub LoadBusinessCentersDataTab()
        Try

            Dim oCurrentBusinessCenter As roBusinessCenter

            If Request("ID") = "-1" Then
                oCurrentBusinessCenter = New roBusinessCenter
            Else
                oCurrentBusinessCenter = TasksServiceMethods.GetBusinessCenterByID(Nothing, Request("ID"), False)
            End If
            ...
            img.Src = VirtualPathUtility.ToAbsolute("~/Tasks/Images/BusinessCenters80.png")
            oImageDiv.Controls.Add(img)

            Dim oTextDiv As New HtmlGenericControl("div")
            oTextDiv.Attributes("class") = "blackRibbonDescription"
            oTextDiv.InnerHtml = "<div id=""NameReadOnly""><span id=""readOnlyNameBusinessCenters"" class=""NameText"">" & oCurrentBusinessCenter.Name & " </span></div>"
    ...
    Private Sub GetBarButtons(ByVal sID As String)
        Try
            Dim guiActions As Generic.List(Of roGuiAction) = PortalServiceMethods.GetGuiActions(Nothing, "Portal\CostControl\BusinessCenters\Management", WLHelperWeb.CurrentPassportID)
            Dim oResponse As roTools.PageCustomActions = roTools.BuildPageCustomActions(guiActions, sID, Me.Language, Me.DefaultScope, "BusinessCenters")
    ...
    ''' <summary>
    ''' Elimina BusinessCenter
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub DeleteBusinessCenterDataX(ByVal oID As Integer)
        Try
            ...
            If TasksServiceMethods.DeleteBusinessCenterByID(Nothing, oID, True) = False Then
                rError = New roJSON.JSONError(True, roWsUserManagement.SessionObject.States.BusinessCenterState.ErrorText)
            Else
    ...
    Private Function CreateTabs(ByRef intActiveTab As Integer) As HtmlTable
        ...
        oTabButtons(1) = CreateNewHtmlAnchor("TABBUTTON_01", Me.Language.Translate("tabZones", Me.DefaultScope), "bTab")

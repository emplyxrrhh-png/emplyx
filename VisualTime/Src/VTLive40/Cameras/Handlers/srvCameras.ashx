<%@ WebHandler Language="VB" Class="srvCameras" %>
...
Public Class srvCameras
    ...
    Private Sub LoadCamerasDataTab()
        Try
            ...
            Else
                oCurrentCamera = API.CameraServiceMethods.GetCameraByID(Nothing, roTypes.Any2Integer(Request("ID")), False)
            End If

            If oCurrentCamera Is Nothing Then Exit Sub

            Dim intActiveTab As Integer = roTypes.Any2Integer(Request("aTab"))
            If intActiveTab > 0 Then intActiveTab = 0

            Dim name As String = String.Empty
            If oCurrentCamera.Name IsNot Nothing Then name = oCurrentCamera.Name
            ...
            img.Src = VirtualPathUtility.ToAbsolute("~/Cameras/Images/Cameras80.png")
            oImageDiv.Controls.Add(img)

            Dim oTextDiv As New HtmlGenericControl("div")
            oTextDiv.Attributes("class") = "blackRibbonDescription"
            oTextDiv.InnerHtml = "<div id=""NameReadOnly""><span id=""readOnlyNameCamera"" class=""NameText"">" & name & " </span></div>"
    ...
    Private Sub GetBarButtons(ByVal sID As String)
        Try
            Dim guiActions As Generic.List(Of roGuiAction) = PortalServiceMethods.GetGuiActions(Nothing, "Portal\GeneralManagement\Cameras\Management", WLHelperWeb.CurrentPassportID)

            Dim oResponse As roTools.PageCustomActions = roTools.BuildPageCustomActions(guiActions, sID, Me.Language, Me.DefaultScope, "Cameras")
    ...
    Private Sub DeleteCamera(ByVal oID As Integer)
        Try
            ...
            If API.CameraServiceMethods.DeleteCamera(Nothing, oID, True) = False Then
                rError = New roJSON.JSONError(True, roWsUserManagement.SessionObject.States.CameraState.ErrorText)
            Else

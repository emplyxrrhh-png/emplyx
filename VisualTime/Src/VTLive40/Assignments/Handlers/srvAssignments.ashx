<%@ WebHandler Language="VB" Class="srvAssignments" %>
...
Public Class srvAssignments
    ...
    Public Overrides Sub ProcessRoboticsRequest(ByVal context As HttpContext)
        ...
        If oPermission > Permission.None Then
            Select Case Request("action")
                Case "getAssignmentsTab"
                    LoadAssignmentsDataTab()
                Case "deleteAssignment"
                    DeleteAssignment(Request("ID"))
                Case "getBarButtons"
    ...
    Private Sub LoadAssignmentsDataTab()
        Try
            ...
            Else
                oCurrentAssignment = AssignmentServiceMethods.GetAssignment(Nothing, Request("ID"), False)
            End If

            If oCurrentAssignment Is Nothing Then Exit Sub
            ...
            img.Src = "Images/Assignments80.png"
            oImageDiv.Controls.Add(img)

            Dim oTextDiv As New HtmlGenericControl("div")
            oTextDiv.Attributes("class") = "blackRibbonDescription"
            oTextDiv.InnerHtml = "<div id=""NameReadOnly""><span id=""readOnlyNameAssignments"" class=""NameText"">" & oCurrentAssignment.Name & " </span></div>"
    ...
    Private Sub GetBarButtons(ByVal sID As String)
        Try
            ...
            Dim guiActions As Generic.List(Of roGuiAction) = PortalServiceMethods.GetGuiActions(Nothing, "Portal\ShiftManagement\Assignments\Management", WLHelperWeb.CurrentPassportID)

            Dim oResponse As roTools.PageCustomActions = roTools.BuildPageCustomActions(guiActions, sID, Me.Language, Me.DefaultScope, "Assignments")
    ...
    Private Sub DeleteAssignment(ByVal oID As Integer)
        Try
            ...
            If AssignmentServiceMethods.DeleteAssignment(Nothing, oID, True) = False Then
                rError = New roJSON.JSONError(True, roWsUserManagement.SessionObject.States.AssignmentState.ErrorText)
            Else

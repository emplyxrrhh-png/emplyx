...
function cargaShiftTabSuperior(IDShift) {
    try {
        ...
        AjaxCall("POST", "json", "../../../Shifts/Handlers/srvShiftsV2.ashx", parms, "CONTAINER", "divShifts", "cargaShiftBarButtons(" + IDShift + ")");        
    }
    catch (e) {
...
function cargaShiftBarButtons(IDShift) {
    try {
        ...
        Url = "../../../Shifts/Handlers/srvShiftsV2.ashx?action=getBarButtons&ID=" + IDShift;
...
function changeTabsShifts(numTab) {
    ...
    arrDivs = new Array('div00', 'div01', 'div02');
...
function hasChangesShifts(bolChanges, markRecalc) {
    ...
    var divButtons = document.getElementById('actionButtons');
    ...
    if (bolChanges) {                        
        ...
        divContenido.style.height = "calc(99% - 37px)";
    } else {                        
        divButtons.style.display = 'none';
        divContenido.style.height = "99%";
    }
}
...
function ShiftTypeChanged() {
    ...
    var arrButtons = new Array('TABBUTTON_00', 'TABBUTTON_01', 'TABBUTTON_02', 'TABBUTTON_03', 'TABBUTTON_04', 'TABBUTTON_05');
    ...
    switch (curShiftType) {
        case 'Normal':
            ...
            hideVacationProperties();
            hideFloatingProperties();
            break;
        case 'Vacations':
...
function retrieveRuleDescription(arrFields, destID) {
    try {
        ...
        showLoadingGrid(true);
        AsyncCall("POST", "../../../Shifts/Handlers/srvShiftsV2.ashx?action=retRuleDesc&" + oFields, "json", "arrStatus", "showLoadingGrid(false);checkShiftStatus(arrStatus); if(arrStatus[0].error == 'false'){ putDescriptionRule(arrStatus,'" + destID + "'); }");
...
function isUsed() {
    try {
        showLoadingGrid(true);
        AsyncCall("POST", "../../../Shifts/Handlers/srvShiftsV2.ashx?action=shiftIsUsed&ID=" + actualShift, "json", "arrStatus", "showLoadingGrid(false); checkShiftStatus(arrStatus); if(arrStatus[0].error == 'false'){ retIsUsed(arrStatus); }")
...
function copyShift() {
    try {
        if (actualShift == -1) {
            ...
            showLoadingGrid(true);
            AsyncCall("POST", "../../../Shifts/Handlers/srvShiftsV2.ashx?action=copyXShift&ID=" + actualShift, "json", "arrStatus", "showLoadingGrid(false); checkShiftStatus(arrStatus); if(arrStatus[0].error == 'false'){ refreshTree(); }")
        }
...
function deleteShift(oId, usedShift) {
    try {
        ...
        if (usedShift == false) { //No se utiliza, se puede eliminar
            AsyncCall("POST", "../../../Shifts/Handlers/srvShiftsV2.ashx?action=deleteXShift&ID=" + oId, "json", "arrStatus", "checkShiftStatus(arrStatus); if(arrStatus[0].error == 'false'){ deleteSelectedNode(); }")
        } else {
            AsyncCall("POST", "../../../Shifts/Handlers/srvShiftsV2.ashx?action=shiftIsUsedForDel&ID=" + oId, "json", "arrStatus", "showLoadingGrid(false); checkShiftStatus(arrStatus); if(arrStatus[0].error == 'false'){ chkDelIsUsed(arrStatus); }")
        }
...
function cmbConceptBalanceChanged() {
    ...
    if (oId >= 0) {
        ...
        AsyncCall("POST", "../../../Shifts/Handlers/srvShiftsV2.ashx?action=selectedHolidayConceptIsAnualWork&ID=" + oId, "json", "arrStatus", postLoad)
    }
}

function hideVacationProperties() {
    $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_optVacations_panContainer").hide();
}

function showVacationProperties() {
    $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_optVacations_panContainer").show();
}

function hideFloatingProperties() {
    $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_optNormalFloating_panContainer").hide();
}

function showFloatingProperties() {
    $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_optNormalFloating_panContainer").show();
}

function OnShiftNameChange() {
    if (txtShortName_Client.GetValue() == null || txtExportClient.GetValue() == null) {
        var fullName = document.getElementById("ctl00_contentMainBody_ASPxCallbackPanelContenido_txtShiftName_I").value.trim();

        var existingShortNames = (document.getElementsByName("ctl00$contentMainBody$ASPxCallbackPanelContenido$hdnExistingShortNames")[0]?.value || "");

        var shortName = generateShortName(fullName, existingShortNames);

        if (txtShortName_Client.GetValue() == null)
            txtShortName_Client.SetValue(shortName);
        if (txtExportClient.GetValue() == null)
            txtExportClient.SetValue(shortName);
    }
}

function adjustColumnWidths() {
    const rows = document.querySelectorAll("tr.complianceRow");
    let maxWidth = 0;
    ...
    rows.forEach(row => {        
        const width = row.offsetWidth;
        const adjust = maxWidth - width;        
        switch (rowIndex) {
            case 1:
                $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbTotTimeNoSup").width($("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbTotTimeNoSup").width() + adjust);           
                break;    
            case 2:
                $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbGSumRetInf").width($("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbGSumRetInf").width() + adjust);
                break;
            case 3:
                $("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbGSumRetInt").width($("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbGSumRetInt").width() + adjust);
                break;
            case 4:
                $("ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbGSumAntInf").width($("#ctl00_contentMainBody_ASPxCallbackPanelContenido_cmbGSumAntInf").width() + adjust);
                break;
            default:
                break;
        }

...
Partial Class Shifts
    ...
    Protected Sub Page_Init1(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        Me.InsertExtraJavascript("SGroups", "~/Shifts/Scripts/SGroups.js")
        Me.InsertExtraJavascript("ShiftsGroupsV2", "~/Shifts/Scripts/ShiftsGroupsV2.js")
        Me.InsertExtraJavascript("Shifts", "~/Shifts/Scripts/ShiftsV2.js")
        ...
        Me.InsertExtraJavascript("frmNewTypeZone", "~/Shifts/Scripts/frmNewTypeZone.js")
        Me.InsertExtraJavascript("frmAddAssignment", "~/Shifts/Scripts/frmAddAssignment.js")
    ...
    Protected Sub Page_Load1(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.divWorkDayStartBefore.Style("Display") = ""
...
#Region "Shifts"
    ...
    Private Sub LoadShiftCombos()
        ...
        cmbWorkdayStartBefore.Items.Clear()
        cmbWorkdayStartBefore.ValueType = GetType(Integer)
        cmbWorkdayStartBefore.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("WorkdayStart.User", DefaultScope), 0))
        cmbWorkdayStartBefore.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("WorkdayStart.Supervisor", DefaultScope), 1))
        cmbWorkdayStartBefore.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("WorkdayStart.Both", DefaultScope), 2))

        cmbWorkdayStartAfter.Items.Clear()
        cmbWorkdayStartAfter.ValueType = GetType(Integer)
        cmbWorkdayStartAfter.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("WorkdayStart.User", DefaultScope), 0))
        cmbWorkdayStartAfter.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("WorkdayStart.Supervisor", DefaultScope), 1))
        cmbWorkdayStartAfter.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("WorkdayStart.Both", DefaultScope), 2))
        ...
        cmbSoldDisp.Items.Clear()
        cmbSoldDisp.ValueType = GetType(Integer)
        cmbSoldDisp.Items.Add(New DevExpress.Web.ListEditItem(Me.Language.Translate("Concepts.None", DefaultScope), 0))
        ...
        For Each dRC As DataRow In dTblConcepts.Rows
            If dRC("ID") <> 0 Then
                cmbSoldDisp.Items.Add(New DevExpress.Web.ListEditItem(dRC("Name"), dRC("ID")))
        ...
        If CentersDT IsNot Nothing AndAlso CentersDT.Rows.Count > 0 Then
            For Each oRows As DataRow In CentersDT.Rows
                Me.cmbCostCenter.Items.Add(oRows("Name"), oRows("ID"))
            Next
        End If
        ...
        Me.frmAddAssignment1.loadFormAssignment()
    ...
    Private Sub SaveShiftData(ByVal oParameters As ShiftsCallbackRequest)
        ...
        Try
            ...
            oCurrentShift.IsTemplate = chkSchema.Checked
            ...
            oCurrentShift.WebLaboral = optTypePerms.Checked
            oCurrentShift.AdvancedParameters = txtParams.Text
            oCurrentShift.ExportName = txtExport.Text

            If ckNotifyEmployeeExit.Checked Then
                oCurrentShift.EnableNotifyExit = True
                oCurrentShift.NotifyEmployeeExitAt = dtNotifyEmployeeExit.DateTime.Hour * 60 + dtNotifyEmployeeExit.DateTime.Minute

                If ckPunchAtTime.Checked Then
                    oCurrentShift.EnableCompleteExit = True
                    oCurrentShift.CompleteExitAt = dtPunchAtTime.DateTime.Hour * 60 + dtPunchAtTime.DateTime.Minute
                Else
                    oCurrentShift.EnableCompleteExit = False
                    oCurrentShift.CompleteExitAt = 0
                End If
            Else
                oCurrentShift.EnableNotifyExit = False
                oCurrentShift.NotifyEmployeeExitAt = 0
                oCurrentShift.CompleteExitAt = 0
            End If

            If ckWorkdayStartBefore.Checked Then
                oCurrentShift.EnableNotifyBeforeStart = True
                oCurrentShift.NotifyEmployeeBeforeAt = dtWorkdayStartBefore.DateTime.Hour * 60 + dtWorkdayStartBefore.DateTime.Minute
                oCurrentShift.WhoToNotifyBefore = cmbWorkdayStartBefore.SelectedItem.Value
            Else
                oCurrentShift.EnableNotifyBeforeStart = False
                oCurrentShift.NotifyEmployeeBeforeAt = 0
                oCurrentShift.WhoToNotifyBefore = 0
            End If

            If ckWorkdayStartAfter.Checked Then
                oCurrentShift.EnableNotifyAfterStart = True
                oCurrentShift.NotifyEmployeeAfterAt = dtWorkdayStartAfter.DateTime.Hour * 60 + dtWorkdayStartAfter.DateTime.Minute
                oCurrentShift.WhoToNotifyAfter = cmbWorkdayStartAfter.SelectedItem.Value
            Else
                oCurrentShift.EnableNotifyAfterStart = False
                oCurrentShift.NotifyEmployeeAfterAt = 0
                oCurrentShift.WhoToNotifyAfter = 0
            End If
            ...
            oCurrentShift.WebVisible = optChkShowShift.Checked
            ...
            If cmbCostCenter.SelectedItem IsNot Nothing Then
                oCurrentShift.IDCenter = cmbCostCenter.SelectedItem.Value
            Else
                oCurrentShift.IDCenter = 0
            End If

            If Me.opCostOnlyWork.Checked Then
                oCurrentShift.ApplyCenterOnAbsence = False
            Else
                oCurrentShift.ApplyCenterOnAbsence = True
            End If
    ...
    Private Sub LoadShiftData(ByVal oParameters As ShiftsCallbackRequest, Optional ByVal sShift As roShift = Nothing, Optional ByVal bAudit As Boolean = False)
        ...
        Try
            ...
            Me.grdAssignments.Controls.Add(creaGridAssignments())
            ...
            If oCurrentShift.EnableNotifyExit = False Then
                ckNotifyEmployeeExit.Checked = False
                ckPunchAtTime.Checked = False

                ckPunchAtTime.ClientEnabled = False
                CkPunchAtTimeDesc.ClientEnabled = False
                dtPunchAtTime.ClientEnabled = False

                dtNotifyEmployeeExit.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, oCurrentShift.EndLimit.Hour, oCurrentShift.EndLimit.Minute, 0)
                dtPunchAtTime.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 23, 59, 0)
            Else
                ckPunchAtTime.ClientEnabled = True
                CkPunchAtTimeDesc.ClientEnabled = True
                dtPunchAtTime.ClientEnabled = True

                Dim hours As Integer = Fix(oCurrentShift.NotifyEmployeeExitAt / 60)
                ckNotifyEmployeeExit.Checked = True
                dtNotifyEmployeeExit.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, oCurrentShift.NotifyEmployeeExitAt - (hours * 60), 0)
                If oCurrentShift.EnableCompleteExit = False Then
                    ckPunchAtTime.Checked = False
                    dtPunchAtTime.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 23, 59, 0)
                Else
                    ckPunchAtTime.Checked = True
                    hours = Fix(oCurrentShift.CompleteExitAt / 60)
                    dtPunchAtTime.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, oCurrentShift.CompleteExitAt - (hours * 60), 0)
                End If
            End If

            If oCurrentShift.EnableNotifyBeforeStart Then
                Dim hours As Integer = Fix(oCurrentShift.NotifyEmployeeBeforeAt / 60)
                ckWorkdayStartBefore.Checked = True
                dtWorkdayStartBefore.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, oCurrentShift.NotifyEmployeeBeforeAt - (hours * 60), 0)
                cmbWorkdayStartBefore.SelectedItem = cmbWorkdayStartBefore.Items.FindByValue(oCurrentShift.WhoToNotifyBefore)
            Else
                ckWorkdayStartBefore.Checked = False
                dtWorkdayStartBefore.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 0, 0, 0)
                cmbWorkdayStartBefore.SelectedItem = cmbWorkdayStartBefore.Items.FindByValue(oCurrentShift.WhoToNotifyBefore)
                cmbWorkdayStartBefore.ClientEnabled = False
            End If

            If oCurrentShift.EnableNotifyAfterStart Then
                Dim hours As Integer = Fix(oCurrentShift.NotifyEmployeeAfterAt / 60)
                ckWorkdayStartAfter.Checked = True
                dtWorkdayStartAfter.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, oCurrentShift.NotifyEmployeeAfterAt - (hours * 60), 0)
                cmbWorkdayStartAfter.SelectedItem = cmbWorkdayStartAfter.Items.FindByValue(oCurrentShift.WhoToNotifyAfter)
            Else
                ckWorkdayStartAfter.Checked = False
                dtWorkdayStartAfter.Value = roTypes.CreateDateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 0, 0, 0)
                cmbWorkdayStartAfter.SelectedItem = cmbWorkdayStartAfter.Items.FindByValue(oCurrentShift.WhoToNotifyAfter)
                cmbWorkdayStartAfter.ClientEnabled = False
            End If
            ...
            chkSchema.Checked = oCurrentShift.IsTemplate
            ...
            If oCurrentShift.WebLaboral Then
                optTypePerms.Checked = True
                optShiftChange.Checked = False
            Else
                optTypePerms.Checked = False
                optShiftChange.Checked = True
            End If
            ...
            Dim listEditItemCenter As DevExpress.Web.ListEditItem = Me.cmbCostCenter.Items.FindByValue(oCurrentShift.IDCenter)
            If listEditItemCenter Is Nothing Then
                ...
                Dim oCenter As roBusinessCenter = API.TasksServiceMethods.GetBusinessCenterByID(Me, oCurrentShift.IDCenter, False)
                ...
                End If
            Else
                Me.cmbCostCenter.SelectedItem = Me.cmbCostCenter.Items.FindByValue(oCurrentShift.IDCenter)
            End If
            ...
            If oCurrentShift.ApplyCenterOnAbsence Then
                Me.opCostOnlyWork.Checked = False
                Me.opCostAll.Checked = True
            Else
                Me.opCostOnlyWork.Checked = True
                Me.opCostAll.Checked = False
            End If

            Dim bolCostCentersLicense As Boolean = HelperSession.GetFeatureIsInstalledFromApplication("Feature\CostControl")
            ...
            If Me.oPermission < Permission.Write Then
                ...
                panTbAssignments.Visible = False
    ...
    Private Sub ProcessShiftSelectedTabVisible(ByVal oParameters As ShiftsCallbackRequest)
        ...
        Me.div06.Style("display") = "none"
        Me.div07.Style("display") = "none"
        Me.div08.Style("display") = "none"
        Me.div09.Style("display") = "none"
        ...
        Select Case oParameters.aTab
            ...
            Case 6
                Me.div06.Style("display") = ""
            Case 7
                Me.div07.Style("display") = ""
            Case 8
                If Me.isHRScheduling.Value = "1" Then
                    Me.div08.Style("display") = ""
                Else
                    Me.div09.Style("display") = ""
                End If

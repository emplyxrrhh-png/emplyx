VTPortal.SelectListObject = function (params) {
    ...
    var listItemClick = function (info) {
        ...
        switch (params.param) {
            case "E":
                window.VTPortalUtils.utils.doPunch('ta', { idCause: info.itemData.FieldValue.toNumber(), direction: 'E', tcType: tcType(), selectedZone: VTPortal.roApp.SelectedZone() });
                break;
            case "S":
                window.VTPortalUtils.utils.doPunch('ta', { idCause: info.itemData.FieldValue.toNumber(), direction: 'S', tcType: tcType() });
                break;
            case "TCH":
            case "TCO":
    ...
    var onZoneClick = function (e) {
        ...
        quickPunchWithManualZone(e.itemData.Id);
    }

    var quickPunchWithManualZone = function (selectedZone) {
        if (VTPortal.roApp.userStatus().PresenceStatus == "Inside") {
            ...
            VTPortalUtils.utils.questionMessage(i18nextko.i18n.t('roAlreadyIn'), 'warning', 0, function () {
                window.VTPortalUtils.utils.doPunch('ta', { idCause: VTPortal.roApp.SelectedCause(), direction: 'S', tcType: tcType() });
            }, function () {
            }, i18nextko.i18n.t('roContinue'), i18nextko.i18n.t('roCancel'));
        } else {
            window.VTPortalUtils.utils.doPunch('ta', { idCause: VTPortal.roApp.SelectedCause(), direction: 'E', tcType: tcType(), selectedZone: selectedZone });
        }
    };

VTPortal.documents = function (params) {
    ...
    function openSigningWebsite(value, idDocument) {
        if (typeof cordova != 'undefined' && typeof cordova.InAppBrowser != 'undefined') {
            if (typeof VTPortal.roApp.isMT() != 'undefined' && VTPortal.roApp.isMT().toLowerCase() == "true") {
                var returnUrl = btoa("https://vtportal.visualtime.net/SigningPage.aspx?GUID=" + value.GUID + "&MTClient=" + VTPortal.roApp.companyID + "&App=1&IdDocument=" + idDocument);
            }
            else if (VTPortal.roApp.isMTApp() == true) {
                var returnUrl = btoa("https://vtportal.visualtime.net/SigningPage.aspx?GUID=" + value.GUID + "&MTClient=" + VTPortal.roApp.companyID + "&App=1&IdDocument=" + idDocument);
            }
            else {
                var returnUrl = btoa("https://" + VTPortal.roApp.db.settings.wsURL + "/VTPortal/SigningPage.aspx?GUID=" + value.GUID + "&MTClient=" + VTPortal.roApp.companyID + "&App=1&IdDocument=" + idDocument);
            }
            ...
            if (typeof VTPortal.roApp.isMT() != 'undefined' && VTPortal.roApp.isMT().toLowerCase() == "true") {
                var returnURLBrowser = btoa("https://vtportal.visualtime.net/SigningPage.aspx?GUID=" + value.GUID + "&MTClient=" + VTPortal.roApp.companyID + "&App=0&IdDocument=" + idDocument)
            }
            else {
                var returnURLBrowser = btoa("http://" + VTPortal.roApp.db.settings.wsURL + "/VTPortal/SigningPage.aspx?GUID=" + value.GUID + "&MTClient=" + VTPortal.roApp.companyID + "&App=0&IdDocument=" + idDocument)
            }
    ...
    var viewModel = {
        ...
        btnNewDocument: {
            ...
            visible: (VTPortal.roApp.db.settings.ApiVersion >= VTPortalUtils.apiVersion.SendDocuments) && (VTPortal.roApp.ReadMode() == false),
            text: '',

VTPortal.plannedCause = function (params) {
    ...
    var causeHasDocuments = ko.computed(function () {
        return (!VTPortal.roApp.isTimeGate() && documentsDS().length > 0);
    });
    ...
    var eHour = ko.observable(moment().endOf('day').toDate());
    var duration = ko.observable(moment().endOf('day').toDate());
    ...
    if (typeof params.param != 'undefined' && parseInt(params.param, 10) != -1) {
        //minSelectedDate(moment(params.param, 'YYYY-MM-DD'));
        //maxSelectedDate(moment(params.param, 'YYYY-MM-DD'));
        calSelectedDays[moment(params.param, 'YYYY-MM-DD').format("YYYYMMDD")] = true;
    }
        ...
        new WebServiceRobotics(function (result) {
            if (result.Status == window.VTPortalUtils.constants.OK.value) {
                ...
                if (typeof absenceId() != 'undefined' && absenceId() != null && absenceId() > 0) {
                    //Revisamos si el empleado tiene permisos de borrado
                    if (VTPortal.roApp.empPermissions() != null && VTPortal.roApp.empPermissions().Forecast != null) {
                        canDelete(VTPortal.roApp.empPermissions().Forecast);
                    }
                }
                ...
                iHour(moment(result.strFromTime, "YYYY-MM-DD HH:mm:ss").toDate());
                eHour(moment(result.strToTime, "YYYY-MM-DD HH:mm:ss").toDate());

                duration(moment(result.strHours, "HH:mm").toDate());
    ...
    function viewShown() {
        ...
        if (VTPortal.roApp.db.settings.ApiVersion >= VTPortalUtils.apiVersion.Holidays) {
            ...
            if (VTPortal.roApp.db.settings.ApiVersion >= VTPortalUtils.apiVersion.CausesPermissions) loadLists('causes.programmedCause');
    ...
    function saveRequest() {
        if (minSelectedDate() == null || maxSelectedDate == null) {
            ...
            var onWSResult = function (result) {
                if (result.Status == window.VTPortalUtils.constants.OK.value) {
                    VTPortal.roApp.redirectAtRequestList(9);
                    VTPortalUtils.utils.notifyMesage(i18nextko.i18n.t("roRequestSaved"), 'success', 2000);
                } else {
                    var onContinue = function () {
                        wsRobotics.saveIncidence(minSelectedDate() == null ? VTPortalUtils.nullDate : minSelectedDate(), maxSelectedDate() == null ? VTPortalUtils.nullDate : maxSelectedDate(), strFromTime, strToTime, moment(duration()).format('HH:mm'), reqValue1(), remarks(), true, document);
                    }
            ...
            wsRobotics.saveIncidence(minSelectedDate() == null ? VTPortalUtils.nullDate : minSelectedDate(), maxSelectedDate() == null ? VTPortalUtils.nullDate : maxSelectedDate(), strFromTime, strToTime, moment(duration()).format('HH:mm'), reqValue1(), remarks(), false, document);
        }
    }
    ...
    function deleteRequest() {
        ...
        if (typeof absenceId() != 'undefined' && absenceId() != null && absenceId() > 0) {
            VTPortalUtils.utils.deleteProgrammedAbsence(absenceId(), "hours"); //Revisar
        }
        else {
    ...
    function changeMaxDuration() {
        var startHour = moment(iHour());
        var endHour = moment(eHour());
        ...
        var fromTime = moment(strFromShift + ' ' + startHour.format("HH:mm") + ':00', "YYYY-MM-DD HH:mm:ss");
        var toTime = moment(strToShift + ' ' + endHour.format("HH:mm") + ':00', "YYYY-MM-DD HH:mm:ss");

        duration(moment('1970-01-30 ' + moment(toTime.diff(fromTime)).utc().format('HH:mm') + ':00').toDate());
    }
    ...
    var viewModel = {
        ...
        viewShown: viewShown,
        title: i18nextko.t('roRequestType_PlannedCauses'),
        lblRemarks: i18nextko.t('roRequestRemarksLbl'),
        lblRequestValue1: i18nextko.t('roSelectAbsence'),
        ...
        lblDuration: i18nextko.t('roRequestDurationLbl'),
        ...
        btnDocuments: {
            ...
            text: '',
            hint: i18nextko.i18n.t('roLblDocument'),
        ...
        btnApprove: {
            onClick: function () { VTPortalUtils.utils.approveRefuseRequest(requestId(), 9, true); },
            text: '',
        ...
        btnRefuse: {
            onClick: function () { VTPortalUtils.utils.approveRefuseRequest(requestId(), 9, false); },
            text: '',
        ...
        beginHour: {
            ...
            value: iHour,
            readOnly: formReadOnly,
            onValueChanged: changeMaxDuration,

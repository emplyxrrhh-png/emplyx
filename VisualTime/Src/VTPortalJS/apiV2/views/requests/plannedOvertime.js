VTPortal.plannedOvertime = function (params) {
    var requestId = ko.observable(null), absenceId = ko.observable(null), remarks = ko.observable('');
    var canSave = ko.observable(false), canDelete = ko.observable(false), viewHistory = ko.observable(false), popupVisible = ko.observable(false), popupDocumentsVisible = ko.observable(false), viewActions = ko.observable(false);
    ...
    function saveRequest() {
        if (minSelectedDate() == null || maxSelectedDate == null) {
            ...
            var onWSResult = function (result) {
                if (result.Status == window.VTPortalUtils.constants.OK.value) {
                    VTPortal.roApp.redirectAtRequestList(14);
                    VTPortalUtils.utils.notifyMesage(i18nextko.i18n.t("roRequestSaved"), 'success', 2000);
                } else {
                    var onContinue = function () {
                        wsRobotics.saveOvertime(minSelectedDate() == null ? VTPortalUtils.nullDate : minSelectedDate(), maxSelectedDate() == null ? VTPortalUtils.nullDate : maxSelectedDate(), strFromTime, strToTime, getMaxDuration(), reqValue1() != null ? reqValue1() : '-1', remarks(), true, document);
                    }
            ...
            wsRobotics.saveOvertime(minSelectedDate() == null ? VTPortalUtils.nullDate : minSelectedDate(), maxSelectedDate() == null ? VTPortalUtils.nullDate : maxSelectedDate(), strFromTime, strToTime, getMaxDuration(), reqValue1() != null ? reqValue1() : '-1', remarks(), false, document);
        }
    }
    ...
    function deleteRequest() {
        ...
        if (typeof absenceId() != 'undefined' && absenceId() != null && absenceId() > 0) {
            VTPortalUtils.utils.deleteProgrammedAbsence(absenceId(), "overtime");
        }
        else {
    ...
    function getMaxDuration() {
        ...
        return moment(toTime.diff(fromTime)).utc().format('HH:mm');
    }
    ...
    var viewModel = {
        ...
        viewShown: viewShown,
        title: i18nextko.t('roRequestType_PlannedOvertime'),
        lblRemarks: i18nextko.t('roRequestRemarksLbl'),
        lblRequestValue1: i18nextko.t('roSelectOvertime'),
        ...
        btnApprove: {
            onClick: function () { VTPortalUtils.utils.approveRefuseRequest(requestId(), 14, true); },
            text: '',
        ...
        btnRefuse: {
            onClick: function () { VTPortalUtils.utils.approveRefuseRequest(requestId(), 14, false); },
            text: '',

...
AS
	...
	@yearStart datetime2 = DATEFROMPARTS(YEAR(@startDate), 1, 1),
	@yearEnd datetime2 = DATEADD(DAY, -1, DATEADD(YEAR, 1, DATEFROMPARTS(YEAR(@startDate), 1, 1)))
	...
	insert into @companyValues exec dbo.GetCompanysFieldValue 'USR_Iberper_Comedor';
	...
	select distinct 
		dbo.UFN_SEPARATES_COLUMNS(emp.FullGroupName,1,'\') AS CompanyName, 
		cv.userFieldValue As CompanyInfo,
...
EXECUTE dbo.Report_mapadoferias 1,'20,29,33','33','20240515'
GO
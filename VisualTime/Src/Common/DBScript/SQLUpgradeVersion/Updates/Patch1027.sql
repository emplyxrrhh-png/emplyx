...
CREATE OR ALTER PROCEDURE [dbo].[WebLogin_CredentialExists]  
(  
...
(a.Credential = @credential OR (TRY_CAST(a.credential AS numeric(28,0)) IN (select IDCard from cardaliases where convert(numeric(38,0),RealValue) = convert(numeric(38,0),REPLACE(@credential,'.','')))))  
AND  
...
UPDATE sysroParameters SET Data='1027' WHERE ID='DBVersion'
GO

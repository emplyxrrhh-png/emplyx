...
SELECT   
    ...
    e.Value AS RawValue,  
    s.FieldType,  
    CASE WHEN s.FieldType = 0 THEN CAST(e.Value AS VARCHAR(MAX)) END AS ValueText,  
    ...
    CASE WHEN s.FieldType = 5 THEN CAST(e.Value AS VARCHAR(MAX)) END AS ValueList  
...
UPDATE sysroParameters SET Data='1010' WHERE ID='DBVersion'
GO

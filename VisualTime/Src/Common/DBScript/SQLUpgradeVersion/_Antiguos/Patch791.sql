...
WHERE (select count(*) from sysrovwSecurity_PermissionOverEmployeeAndFeature as pef where pef.IdFeature = 2 and dbo.sysroEmployeesShifts.CurrentDate between pef.BeginDate and pef.EndDate and pef.idpassport = @pidpassport and pef.idemployee = dbo.sysroEmployeesShifts.IDEmployee and pef.FeaturePermission > 0) > 0  
...
--Planificacion y Fichajes - Genius_ScheduleAndPunches
 ...
 ALTER   PROCEDURE [dbo].[Genius_ScheduleAndPunches]
    ...
    where (select count(*) from sysrovwSecurity_PermissionOverEmployeeAndFeature as pef where pef.IdFeature = 2 and alldays.dt between pef.BeginDate and pef.EndDate and pef.idpassport = @pidpassport and pef.idemployee = emp.ID and pef.FeaturePermission > 0) > 0  
    ) reqReg 
   ...
   (select count(*) from punches where IDEmployee = emp.ID and ShiftDate = ds.Date and ActualType = 1) as InPunches,
    (select count(*) from punches where IDEmployee = emp.ID and ShiftDate = ds.Date and ActualType = 2) as OutPunches
              ...
              INNER JOIN dbo.EmployeeContracts with (nolock) ON dbo.EmployeeContracts.IDEmployee = emp.ID AND ds.Date between dbo.EmployeeContracts.BeginDate and dbo.EmployeeContracts.EndDate
                 ...
                 WHERE ds.Date between @pinitialDate and @pendDate AND (select count(*) from sysrovwSecurity_PermissionOverEmployeeAndFeature as pef where pef.IdFeature = 2 and ds.Date between pef.BeginDate and pef.EndDate and pef.idpassport = @pidpassport and pef.idemployee = emp.ID and pef.FeaturePermission > 0) > 0  
...
UPDATE sysroParameters SET Data='791' WHERE ID='DBVersion'
GO

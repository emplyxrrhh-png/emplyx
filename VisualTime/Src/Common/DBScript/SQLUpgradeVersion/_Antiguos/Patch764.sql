 ...
 BEGIN
  ...
  select @conceptNames = substring(tmp.conceptNames,2, LEN(tmp.conceptNames) -1) from 
   ...
   SET @SqlStatement = N'
	...
	LEFT OUTER JOIN sysrovwEmployeesAnnualWorkPeriods with (nolock) ON sysrovwEmployeesAnnualWorkPeriods.IdEmployee = emp.ID and accruals.Date between sysrovwEmployeesAnnualWorkPeriods.beginperiod and sysrovwEmployeesAnnualWorkPeriods.endperiod
...
UPDATE sysroParameters SET Data='764' WHERE ID='DBVersion'
GO

CREATE PROCEDURE [dbo].[Report_quadrant_by_schedule_presence]
...
CASE WHEN PA1.IDEmployee IS NULL
	...
	CASE WHEN PH1.AllDay = 0 THEN 1 ELSE NULL END AS ISABSENTHOURS1,
	CASE WHEN PH2.AllDay = 0 THEN 2 ELSE NULL END AS ISABSENTHOURS2,
	CASE WHEN PH3.AllDay = 0 THEN 3 ELSE NULL END AS ISABSENTHOURS3,
	CASE WHEN PH4.AllDay = 0 THEN 4 ELSE NULL END AS ISABSENTHOURS4,
	CASE WHEN PH5.AllDay = 0 THEN 5 ELSE NULL END AS ISABSENTHOURS5,
	CASE WHEN PH6.AllDay = 0 THEN 6 ELSE NULL END AS ISABSENTHOURS6,
	CASE WHEN PH7.AllDay = 0 THEN 7 ELSE NULL END AS ISABSENTHOURS7,
	E.Name,
	CASE WHEN S.ExpectedWorkingHours > 0 THEN 1 ELSE NULL END AS ISWORKINGSHIFT
FROM DailySchedule AS DS
INNER JOIN EMPLOYEES AS E ON E.ID = DS.IDEmployee
INNER JOIN SHIFTS AS S2 ON S2.ID = @IDShift AND S2.ExpectedWorkingHours > 0
...
UPDATE sysroParameters SET Data='971' WHERE ID='DBVersion'
GO

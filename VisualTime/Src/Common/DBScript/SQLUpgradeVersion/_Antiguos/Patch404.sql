-- Creamos nuevo campo en las justificaciones diarias para indicar si se ha generado a partir de una regla diaria
ALTER TABLE dbo.DailyCauses DROP CONSTRAINT [PK_DailyCauses]
GO

ALTER TABLE [dbo].[DailyCauses] ADD [DailyRule] [smallint] NOT  NULL DEFAULT(0)
GO

UPDATE [dbo].[DailyCauses] Set DailyRule= 0 WHERE DailyRule IS NULL
GO
...
-- El informe de emergencia mostraba empleados cuyo último fichaje era un acceso denegado
...
--Añadimos nolock a las analiticas
    ...
    SELECT dbo.sysroEmployeeGroups.IDEmployee AS IDEmployee, dbo.sysroEmployeeGroups.GroupName AS GroupName, dbo.sysroEmployeeGroups.IDGroup AS IDGroup, dbo.sysroEmployeeGroups.FullGroupName As FullGroupName, dbo.sysroEmployeeGroups.Path AS GroupPath, 
        ...
        FROM dbo.sysroEmployeeGroups with (nolock) 
         ...
         INNER JOIN dbo.AccessGroups with (nolock) ON dbo.AccessGroups.ID IN
                        ...
                        FROM            dbo.EmployeeAccessAuthorization with (nolock)
 ...
 ALTER PROCEDURE [dbo].[Analytics_CostCenters]
    ...
    AS
     ...
     FROM         dbo.sysroEmployeeGroups with (nolock) 
      ...
      where     (dbo.businesscenters.id is null or dbo.businesscenters.id in (select distinct idcenter from sysropassports_centers with (nolock) where idpassport=(SELECT isnull(IdparentPassport,0) from sysroPassports with (nolock) where ID = @idpassport)) )
  ...
  ALTER PROCEDURE [dbo].[Analytics_Tasks]
   ...
   AS
    ...
    FROM         dbo.sysroEmployeeGroups with (nolock)  
  						...
  						INNER JOIN dbo.Employees with (nolock) ON dbo.sysroEmployeeGroups.IDEmployee = dbo.Employees.ID
    WHERE     (dbo.businesscenters.id is null OR dbo.BusinessCenters.ID IN (SELECT DISTINCT IDCenter FROM sysroPassports_Centers with (nolock) WHERE IDPassport=(SELECT isnull(IdparentPassport,0) from sysroPassports where ID = @idpassport)) )
...
INSERT INTO dbo.sysroLiveAdvancedParameters ([ParameterName],[Value]) VALUES ('CentesimalFormatProductiV','0')
GO



UPDATE dbo.sysroParameters SET Data='404' WHERE ID='DBVersion'
GO

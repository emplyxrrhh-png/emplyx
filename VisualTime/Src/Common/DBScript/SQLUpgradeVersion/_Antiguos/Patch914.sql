  ...
  select @conceptNamesWithZero = substring(tmp.conceptNames,2, LEN(tmp.conceptNames) -1) from 
   ...
   SET @SqlStatement = N'
 	...
 	SELECT 
 		...
 		' + @conceptNamesWithZero + ',' + @conceptNamesWithZeroToHours + ',' +  iif(@annualConceptNamesWithZero is null,'',@annualConceptNamesWithZero + ',') + iif(@annualConceptNamesWithZeroToHours is null,'',@annualConceptNamesWithZeroToHours + ',') + ' dbo.GetPunchesByEmployeeAndDate(emp.ID,accruals.Date) as punches,
...
UPDATE sysroParameters SET Data='914' WHERE ID='DBVersion'
GO

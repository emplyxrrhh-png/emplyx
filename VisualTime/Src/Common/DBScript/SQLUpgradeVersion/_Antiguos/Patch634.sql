insert into sysroLiveAdvancedParameters select 'OptionsParameters.MaxMovementHours', DATEDIFF(mi,CONVERT(time,'00:00'), CONVERT(time,a.UntilUserFieldFROM))/60.0 from 
(SELECT isnull (CONVERT(xml, sysroParameters.Data).value('(/roCollection/Item[@key=("MaxMovementHours")]/text())[1]', 'nvarchar(max)'), '12:00')
as UntilUserFieldFROM from sysroParameters where ID='OPTIONS' ) a
go
...
UPDATE sysroParameters SET Data='634' WHERE ID='DBVersion'
GO

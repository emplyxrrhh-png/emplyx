...
dbo.Requests.RequestType, isnull(CONVERT(NVARCHAR(4000), dbo.Requests.Date1, 120),'') as 'BeginDateRequest',  
CASE WHEN Requests.FromTime IS NOT NULL THEN isnull(CONVERT(NVARCHAR(4000), dbo.Requests.FromTime, 8),'')
ELSE isnull(CONVERT(NVARCHAR(4000), RD.BEGINTIME, 8),'') END as 'BeginHourRequest',
isnull(CONVERT(NVARCHAR(4000), isnull(dbo.Requests.Date2,dbo.Requests.Date1), 120),'') as 'EndDateRequest',
CASE WHEN REQUESTS.FromTime IS NOT NULL THEN isnull(CONVERT(NVARCHAR(4000), dbo.Requests.ToTime, 8),'')
ELSE isnull(CONVERT(NVARCHAR(4000), RD.EndTime, 8),'')
END as 'EndHourRequest',
...
LEFT JOIN sysroRequestDays AS RD ON RD.IDRequest = Requests.ID
...
UPDATE sysroParameters SET Data='844' WHERE ID='DBVersion'

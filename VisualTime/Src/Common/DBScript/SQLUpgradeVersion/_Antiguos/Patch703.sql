...
-- 04. En el caso que la fecha de inicio de contrato sea 29/02, la fecha de fin de tramo siempre sera 28/02 del año siguiente siempre y cuando no sea superior al fin del contrato
select IDEmployee, BeginPeriod, case when dbo.Is0229(begindate)=1 then case when DATEFROMPARTS(year(beginperiod)+1,2,28) < EndPeriod then DATEFROMPARTS(year(beginperiod)+1,2,28) else EndPeriod end Else EndPeriod end  as EndPeriod  from (
...
UPDATE sysroParameters SET Data='703' WHERE ID='DBVersion'
GO

...
ALTER PROCEDURE [dbo].[Genius_Authorizations]
  		...
  		dbo.GetEmployeeAge(sysroEmployeeGroups.idEmployee) As Age   
			...
			INNER JOIN sysrovwSecurity_PermissionOverEmployeeAndFeature as pef on pef.IdFeature = 9 and CAST(GETDATE() AS DATE) between pef.BeginDate and pef.EndDate and pef.idpassport = @idpassport and pef.idemployee = Employees.ID and FeaturePermission > 0
   		...
   		WHERE dbo.sysroEmployeeGroups.IDEmployee in( select idEmployee from @employeeIDs) AND getdate() between sysroEmployeeGroups.BeginDate and sysroEmployeeGroups.EndDate  
...
LEFT OUTER JOIN sysrovwEmployeesAnnualWorkPeriods ON sysrovwEmployeesAnnualWorkPeriods.IdEmployee = DailyAccruals.IDEmployee and dbo.DailyAccruals.date between sysrovwEmployeesAnnualWorkPeriods.beginperiod and sysrovwEmployeesAnnualWorkPeriods.endperiod AND DBO.Concepts.DefaultQuery = 'L'
) AS AccrualsPrice
ON
...
UPDATE sysroParameters SET Data='772' WHERE ID='DBVersion'
GO

...
CREATE OR ALTER FUNCTION [dbo].[sysrofnSecurity_PermissionsOverDocumentTemplates]
   ...
   BEGIN
			...
			INNER JOIN sysroDocumentLOPDLevels ON sysroDocumentLOPDLevels.Id = DocumentTemplates.LOPDAccessLevel
			INNER JOIN sysrovwSecurity_PermissionOverGroups POG ON (POG.IdGroup = @idcompanypar OR @idcompanypar = 0) AND (POG.IdPassport = @idpassportpar OR @idpassportpar = 0) AND POG.IsCompany = 1
...
UPDATE sysroParameters SET Data='842' WHERE ID='DBVersion'

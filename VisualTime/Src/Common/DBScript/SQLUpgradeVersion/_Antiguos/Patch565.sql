...
IF (SELECT COUNT(*) FROM DBO.GeniusViews WHERE IdPassport = 0 AND NAME = 'punchesWithUnreliableZone') = 0
BEGIN
...
INSERT INTO [dbo].[GeniusViews]
           ...
           ,[IdParentShared])
     VALUES
           (0,'punchesWithUnreliableZone','	',	'P',	1,	'20220613',	'',	0,	'20220613',	'20220613',	'{"slice":{"reportFilters":[{"uniqueName":"IDContract"},{"uniqueName":"CurrentEmployee"}],"rows":[{"uniqueName":"GroupName"},{"uniqueName":"FullGroupName"},{"uniqueName":"EmployeeName"},{"uniqueName":"Date_ToDateString"},{"uniqueName":"Time"},{"uniqueName":"IsNotReliable"},{"uniqueName":"ZoneIsNotReliable","filter":{"members":["zoneisnotreliable.[no fiable]"]}}],"columns":[{"uniqueName":"[Measures]"}],"measures":[{"uniqueName":"Value","aggregation":"sum","caption":"Total"}],"expands":{"expandAll":true}},"options":{"grid":{"type":"classic","showTotals":"off"}},"creationDate":"2022-06-13T18:27:39.223Z"}',	'',	'',	'',	'{"IncludeZeros":true,"IncludeZeroBusinessCenter":true,"LanguageKey":"ESP","RequestTypes":""}',	'Genius_Punches(@initialDate,@endDate,@idpassport,@employeeFilter,@userFieldsFilter))',	'Calendar',	'Calendar.Analytics',NULL,NULL)           
...
UPDATE sysroParameters SET Data='565' WHERE ID='DBVersion'
GO
...
DROP VIEW IF EXISTS [dbo].[sysrovwDaysAbsences]
GO
CREATE VIEW [dbo].[sysrovwDaysAbsences]
    AS
		SELECT IdCause, IDEmployee, Date1 as BeginDate, Date2 as FinishDate, 0 As MaxLastingDays, ID as AbsenceID, 1 as IsRequest 
		FROM Requests 
		WHERE RequestType = 7 AND (Status = 0 OR Status = 1)
    UNION
		SELECT IdCause, IDEmployee, BeginDate, FinishDate, MaxLastingDays, AbsenceID, 0 as IsRequest
		FROM ProgrammedAbsences 
GO
DROP VIEW IF EXISTS [dbo].[sysrovwHoursAbsences]
GO
 CREATE VIEW [dbo].[sysrovwHoursAbsences]
    AS
    ...
    UNION
		SELECT IdCause, IDEmployee, Date, FinishDate, BeginTime, EndTime, Duration, AbsenceID, 1 as IsRequest
...
ALTER VIEW [dbo].[sysrovwForecastDocumentsFaults]
    ...
    left join (Select row_number() over (partition by IdEmployee, IdDocumentTemplate, IdHoursAbsence, IdRequest order by IdDocumentTemplate desc) As 'RowNumber1', * from documents where Status = 0 or Status = 1) docpending on docpending.IdDocumentTemplate = docsabs.documenttemplateid and (docpending.IdHoursAbsence = pc.AbsenceID OR docpending.IdRequest = pc.AbsenceID) and (docpending.Status = 0 or (docpending.Status= 1 and docpending.StatusLevel > docsabs.ApprovalLevelRequired))
    Left join (Select row_number() over (partition by IdEmployee, IdDocumentTemplate order by deliverydate desc) As 'RowNumber', * from documents) doc on doc.IdDocumentTemplate = docsabs.documenttemplateid
...
UPDATE sysroParameters SET Data='498' WHERE ID='DBVersion'
GO

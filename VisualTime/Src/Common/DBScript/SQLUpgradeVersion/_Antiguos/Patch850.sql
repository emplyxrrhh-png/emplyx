...
(a.Credential = @credential OR (a.credential IN (select convert(nvarchar,IDCard) from cardaliases where convert(numeric(38,0),RealValue) = convert(numeric(38,0),REPLACE(@credential,'.','')))))
AND
...
UPDATE sysroParameters SET Data='850' WHERE ID='DBVersion'

...
ALTER VIEW [dbo].[sysrovwNonSupervisedDepartments]
AS
SELECT g.id, g.name, g.path, g.fullgroupname
FROM groups AS g
LEFT JOIN (
SELECT DISTINCT g.path AS auxpath
FROM sysroPassports_Groups AS spg
INNER JOIN groups AS g ON g.ID = spg.IDGroup
INNER JOIN sysroPassports AS sp ON sp.id = spg.IDPassport AND sp.IsSupervisor = 1
...
UPDATE sysroParameters SET Data='675' WHERE ID='DBVersion'
GO

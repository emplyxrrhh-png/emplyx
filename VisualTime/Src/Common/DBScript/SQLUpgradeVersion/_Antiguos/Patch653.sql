...
ADD Causes NVARCHAR(4000)

GO

UPDATE GeniusViews SET DSFunction = 'Genius_ConceptsAndScheduleAndIncidencesAndPunches(@initialDate,@endDate,@idpassport,@employeeFilter,@conceptsFilter,@userFieldsFilter,@includeZeros,@causesFilter))'
WHERE DSFunction LIKE 'Genius_ConceptsAndScheduleAndIncidencesAndPunches(%'
...
ALTER PROCEDURE [dbo].[Genius_ConceptsAndScheduleAndIncidencesAndPunches]
   ...
   insert into @employeeIDs exec dbo.ObtainEmployeeIDsFromFilter @pemployeeFilter,@initialDate,@pendDate    
	...
	if (@causesFilter = -1 OR @causesFilter is NULL OR @causesFilter = '')
	begin
...
UPDATE GeniusViews
SET DSFunction = 'Genius_ConceptsAndScheduleAndIncidences(@initialDate,@endDate,@idpassport,@employeeFilter,@conceptsFilter,@userFieldsFilter,@includeZeros,@causesFilter)'
WHERE DSFunction LIKE 'Genius_ConceptsAndScheduleAndIncidences(%'
...
UPDATE GeniusViews
SET DSFunction = 'Genius_Incidences(@initialDate,@endDate,@idpassport,@employeeFilter,@userFieldsFilter,@causesFilter)'
WHERE DSFunction LIKE 'Genius_Incidences(%'
...
UPDATE GeniusViews
SET DSFunction = 'Genius_IncidencesByTime(@initialDate,@endDate,@idpassport,@employeeFilter,@userFieldsFilter,@initialTime,@endTime,@causesFilter)'
WHERE DSFunction LIKE 'Genius_IncidencesByTime(%'

GO

ALTER PROCEDURE [dbo].[Genius_CostCenters_Detail]
           @initialDate smalldatetime, @endDate smalldatetime, @idpassport int, @employeeFilter nvarchar(max), @userFieldsFilter nvarchar(max), @costCenterFilter nvarchar(max), @causesFilter nvarchar(max)   AS  
   		DECLARE @employeeIDs Table(idEmployee int)  
...
UPDATE GeniusViews
SET DSFunction = 'Genius_CostCenters_Detail(@initialDate,@endDate,@idpassport,@employeeFilter,@userFieldsFilter,@costCenterFilter,@causesFilter,@includeZeros)'
WHERE DSFunction LIKE 'Genius_CostCenters_Detail(%'
...
UPDATE sysroParameters SET Data='653' WHERE ID='DBVersion'
GO

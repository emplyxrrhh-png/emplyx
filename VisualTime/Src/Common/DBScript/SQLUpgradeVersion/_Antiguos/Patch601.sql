--ACTUALIZAMOS INFORMES DX
ALTER TABLE GeniusViews
ADD RequieredLicense NVARCHAR(50) NULL

GO

DROP PROCEDURE [Genius_TelecommutingAgreementCompliance]

GO

CREATE PROCEDURE [dbo].[Genius_TelecommutingAgreementCompliance]
   ...
   CASE WHEN sysrovwEmployeeTelecommuting.TelecommuteOptionalExpected = 1 OR sysrovwEmployeeTelecommuting.TelecommuteOptionalPlanned = 1 THEN 'Opcional' 
   WHEN sysrovwEmployeeTelecommuting.TelecommuteExpected = 1 OR sysrovwEmployeeTelecommuting.TelecommutePlanned = 1 THEN 'Teletrabajo' 
   ...
   EffectiveTCWork.Value AS EffectiveTCTime,
   EffectivePresenceWork.Value AS EffectivePresenceTime,
   ...
   CASE WHEN sysrovwEmployeeTelecommuting.TelecommuteOptionalExpected = 1 OR sysrovwEmployeeTelecommuting.TelecommuteOptionalPlanned = 1 THEN 2 
   WHEN sysrovwEmployeeTelecommuting.TelecommuteExpected = 1 OR sysrovwEmployeeTelecommuting.TelecommutePlanned = 1 THEN 1
...
DELETE FROM GeniusViews WHERE IdPassport = 0 AND NAME = 'telecommutingAgreementCompliance'

GO

IF (SELECT COUNT(*) FROM DBO.GeniusViews WHERE IdPassport = 0 AND NAME = 'telecommutingAgreementCompliance') = 0
BEGIN
...
INSERT INTO [dbo].[GeniusViews]
           ...
           ,[RequieredLicense])
     VALUES
           (0,'telecommutingAgreementCompliance','	',	'P',	1,	'20220922',	'',	0,	'20220922',	'20220922',	'{"slice":{"reportFilters":[{"uniqueName":"IDContract"},{"uniqueName":"CurrentEmployee"}],"rows":[{"uniqueName":"EmployeeName"},{"uniqueName":"Mes"},{"uniqueName":"Date_ToDateString"},{"uniqueName":"PlannedDayType"},{"uniqueName":"ExpectedWorkingHours(HH:MM)_TOHOURS"},{"uniqueName":"PunchDayType"},{"uniqueName":"EffectivePresenceTime(HH:MM)_TOHOURS"},{"uniqueName":"EffectiveTCTime(HH:MM)_TOHOURS"}],"columns":[{"uniqueName":"[Measures]"}],"measures":[{"uniqueName":"Compliance","formula":"IF((\"IdPunchDayType\" = 0 AND \"IdPlannedDayType\"  = 0) OR (\"IdPunchDayType\" = 1 AND \"IdPlannedDayType\"  = 1) OR (\"IdPlannedDayType\" = 2), 1, 0)"}],"expands":{"expandAll":true}},"options":{"grid":{"type":"classic","showTotals":"off","showGrandTotals":"off"}},"conditions":[{"formula":"#value = 0","measure":"Compliance","format":{"backgroundColor":"#FF0000","color":"#FF0000","fontFamily":"Arial","fontSize":"12px"}},{"formula":"#value = 1","measure":"Compliance","format":{"backgroundColor":"#FFFFFF","color":"#FFFFFF","fontFamily":"Arial","fontSize":"12px"}}],"creationDate":"2022-09-26T11:36:54.677Z"}',	'',	'',	'',	'{"IncludeZeros":false,"IncludeZeroBusinessCenter":false,"LanguageKey":"ESP","RequestTypes":""}',	'Genius_TelecommutingAgreementCompliance(@initialDate,@endDate,@idpassport,@employeeFilter,@userFieldsFilter))',	'Calendar',	'Calendar.Analytics','',-1,'Feature\Telecommuting')           

END
GO

UPDATE GeniusViews SET RequieredLicense = 'Feature\Telecommuting' WHERE NAME IN ('effectiveWorkPresenceVSTele','telecommutingForecastAndTrend')
...
UPDATE sysroParameters SET Data='601' WHERE ID='DBVersion'
GO

...
DROP FUNCTION IF EXISTS [dbo].[sysrofnSecurity_PermissionsOverDocumentTemplates]
 ...
 CREATE FUNCTION [dbo].[sysrofnSecurity_PermissionsOverDocumentTemplates]
   ...
   BEGIN
			...
			INNER JOIN sysroPassports_Categories AS PassportCategory ON PassportCategory.IDPassport = POE.IdPassport AND PassportCategory.IDCategory = DocumentTemplates.Area
			...
			INNER JOIN sysroPassports_Categories AS PassportCategory ON PassportCategory.IDPassport = POG.IdPassport AND PassportCategory.IDCategory = DocumentTemplates.Area
			) AUX1 WHERE AUX1.PermissionOverArea > 0  AND AUX1.PermissionOverLOPDLevel > 0
			) AUX 
 	RETURN
   END
   ...
   BEGIN
			...
			(
				...
				FROM sysroRequestType rt
					...
					INNER JOIN sysroPassports_Categories AS PassportCategory ON PassportCategory.IDPassport = sysroPassports.Id AND PassportCategory.IDCategory = rt.IDCategory
				WHERE  Permission >= 3 AND pove.IdEmployee = @idemployeepar and rt.IdType = @idrequesttypepar
...
UPDATE sysroParameters SET Data='781' WHERE ID='DBVersion'
GO

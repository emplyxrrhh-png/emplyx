ALTER FUNCTION [dbo].[EmployeeZonesBetweenDates]
   ...
   BEGIN
	...
	INSERT INTO @employeeIDs SELECT Value AS IDEmployye FROM SplitToInt(@employees,',')
	...
	ELSE
		...
		SELECT Employees.Id AS IDEmployee,
			   ...
			   CASE WHEN Punches_LastOnPastDates.ActualType IS NOT NULL AND Punches_LastOnPastDates.ActualType <> 2 AND dt >= DATEADD(day, DATEDIFF(Day, 0, GETDATE()), 0) THEN ISNULL(ZoneExpectedOnDate.Name,'?') END  AS ExpectedZone
		FROM   [dbo].[Alldays] (@iniperiod, @endperiod)
			   INNER JOIN Employees WITH(NOLOCK) ON Employees.ID in (SELECT idEmployee from @employeeIDs)
			   LEFT JOIN EmployeeContracts WITH(NOLOCK) ON alldays.dt BETWEEN EmployeeContracts.BeginDate AND EmployeeContracts.EndDate and EmployeeContracts.IDEmployee = Employees.ID
			   LEFT JOIN sysrovwTelecommutingAgreement Agreement ON Agreement.IDContract = EmployeeContracts.IDContract 
			   LEFT JOIN DailySchedule WITH(NOLOCK) ON DailySchedule.Date = alldays.dt AND DailySchedule.IDEmployee = Employees.ID
			   LEFT JOIN Shifts WITH(NOLOCK) ON Shifts.ID = DailySchedule.IDShift1
			   LEFT JOIN (SELECT row_number() OVER (PARTITION BY IdEmployee, Shiftdate ORDER BY DateTime DESC) AS 'RowNumberTC', * FROM Punches WITH(NOLOCK) WHERE IDEmployee in (SELECT idEmployee from @employeeIDs) AND InTelecommute = 1 AND Shiftdate >= @iniperiod) PunchesTC ON PunchesTC.IDEmployee = Employees.ID AND PunchesTC.ShiftDate = dt AND (RowNumberTC = 1 OR RowNumberTC IS NULL)
			   LEFT JOIN (SELECT row_number() OVER (PARTITION BY IdEmployee, Shiftdate ORDER BY DateTime DESC) AS 'RowNumberLastOnDate', * FROM Punches WITH(NOLOCK) WHERE IDEmployee in (SELECT idEmployee from @employeeIDs) AND IDZone > 0 AND ActualType <> 2 AND Shiftdate >= @iniperiod) Punches_LastOnDate ON Punches_LastOnDate.IDEmployee = Employees.ID AND Punches_LastOnDate.ShiftDate = dt AND (RowNumberLastOnDate = 1 OR RowNumberLastOnDate IS NULL)
			   LEFT JOIN (SELECT row_number() OVER (PARTITION BY IdEmployee ORDER BY DateTime DESC) AS 'RowNumberLastOnPastDates', * FROM Punches WITH(NOLOCK) WHERE IDEmployee in (SELECT idEmployee from @employeeIDs) AND IDZone > 0 AND ActualType <> 2 AND Shiftdate >= DATEADD(day,-30,@iniperiod) AND ShiftDate <= DATEADD(day, DATEDIFF(Day, 0, GETDATE()), 0)) Punches_LastOnPastDates ON Punches_LastOnPastDates.IDEmployee = Employees.ID AND (RowNumberLastOnPastDates = 1 OR RowNumberLastOnPastDates IS NULL)
			   LEFT JOIN Zones AS ZoneOnPastDate ON ZoneOnPastDate.ID = Punches_LastOnDate.IDZone
			   LEFT JOIN Zones AS ZoneExpectedOnDate ON ZoneExpectedOnDate.ID = Punches_LastOnPastDates.IDZone
			   LEFT JOIN ProgrammedAbsences WITH(NOLOCK) ON ProgrammedAbsences.IDEmployee = Employees.ID AND dt BETWEEN ProgrammedAbsences.BeginDate AND isnull(ProgrammedAbsences.FinishDate,DATEADD(day,ProgrammedAbsences.maxlastingdays,ProgrammedAbsences.BeginDate))
...
UPDATE sysroParameters SET Data='654' WHERE ID='DBVersion'
GO

alter VIEW [dbo].[sysrovwDashboardEmployeeStatus]   
	...
	where parametername='OptionsParameters.MaxMovementHours')
				...
				svEmployees.IDGroup,  
                 ...
                 CASE WHEN tmpAttendance.DateTime < dateadd(hour, params.maxMovementHours, getdate()) THEN 'Out-Calculated' ELSE CASE tmpAttendance.ActualType WHEN 1 THEN 'In' ELSE 'Out' END END as AttendanceStatus,     
           ...
           ISNULL(Tasks.Project + ' : ' +  Tasks.Name,'') as LastTaskName,      
                 ...
                 CASE WHEN tmpAttendance.DateTime < dateadd(hour, params.maxMovementHours, getdate()) THEN '' ELSE tmpLastPunch.LocationZone END As LocationName,    
                 CASE WHEN tmpAttendance.DateTime < dateadd(hour, params.maxMovementHours, getdate()) THEN '' ELSE Zones.Name END As ZoneName,    
				 ...
				 svEmployees.Path
                 FROM params, sysrovwcurrentemployeegroups svEmployees WITH (NOLOCK)    
...
AS
select distinct punches.IDEmployee, punches.ShiftDate, s.Name, ds.Date, (select top 1 IDContract FROM EmployeeContracts where EmployeeContracts.IDEmployee = e.id order by begindate desc )  as IDContract,(SELECT TOP 1 cast(Value as nvarchar) FROM EmployeeUserFieldValues  WHERE EmployeeUserFieldValues.IDEmployee = e.ID and FieldName = 'NIF' ORDER BY EmployeeUserFieldValues.Date DESC) AS Dni, e.Name as EmployeeName 
...
UPDATE sysroParameters SET Data='906' WHERE ID='DBVersion'
GO

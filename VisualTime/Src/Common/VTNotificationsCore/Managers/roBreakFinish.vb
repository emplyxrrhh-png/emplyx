...
Public Class roNotificationTask_BreakFinish_Manager
    Inherits roNotificationTaskManager

    Public Sub New(ByVal sGUID As String)
        MyBase.New(eNotificationType.BreakFinish, sGUID)
    End Sub
    ...
    Protected Overrides Function GetTranslatedNotificationTexts(ByVal oConf As roNotificationDestinationConfig) As roNotificationItem
        ...
        Try
            ...
            oItem.Subject = roNotificationHelper.Message("Notification.BreakFinish.Subject", Nothing, , oConf.Language)
            oItem.Body = roNotificationHelper.Message("Notification.BreakFinish.Body", Params, , oConf.Language)
            oItem.Content = String.Empty
            oItem.Destination = oConf.Email
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roBreakFinish::Translate::", ex)
            oItem = Nothing
        End Try
        Return oItem
    End Function
    ...
    Public Overrides Function GenerateNotificationTasks(ByVal ads As DataRow) As Boolean
        ...
        Try
                    ...
                    For Each shiftlayer As Shift.roShiftLayer In oShift.Layers
                        ...
                        If shiftlayer.LayerType = roLayerTypes.roLTBreak Then
                            ...
                            If notifyEmployee Then
                                ...
                                ' Cortesía tras finalizar la franja de descanso
                                Dim courtesyTime As TimeSpan = TimeSpan.Parse(roTypes.Any2String(oData.Item(shiftlayer.XmlKey(roXmlLayerKeys.NotificationForUserAfterTime))))
                                ...
                                'Al menos hemos superado alguna de las dos fechas / horas y el IDType también corresponde y no existe la notificación para este usuario + descanso
                                If notificationDoesNotExists AndAlso breakPeriodFinished Then
                                    'Cogemos los punches del Employee
                                    ...
                                    bRet = roNotificationHelper.CreateBreakNotification(dtPunches, courtesyTime, beginRealBreakDate, finishRealBreakDate, eNotificationType.BreakFinish, roTypes.Any2Integer(drShift("IDEmployee")), roTypes.Any2Integer(ads("ID")), beginBreakTimeToNotify, workStartTime)
                                End If
                            End If
                        End If
                    Next
            ...
            End If
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roBreakFinish::GenerateNotificationTasks:: Unexpected error: ", ex)
            bRet = False
        End Try

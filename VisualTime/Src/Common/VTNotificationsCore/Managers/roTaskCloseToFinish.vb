...
Public Class roNotificationTask_Task_Close_to_Finish_Manager
    Inherits roNotificationTaskManager

    Public Sub New(ByVal sGUID As String)
        MyBase.New(eNotificationType.Task_Close_to_Finish, sGUID)
    End Sub
    ...
    Protected Overrides Function GetTranslatedNotificationTexts(ByVal oConf As roNotificationDestinationConfig) As roNotificationItem
        ...
        Try
            ...
            oItem.Subject = roNotificationHelper.Message("Notification.Taskclosetofinish.Subject", Nothing, , oConf.Language)
            oItem.Body = roNotificationHelper.Message("Notification.Taskclosetofinish.Body", Params, , oConf.Language)
            oItem.Content = String.Empty
            oItem.Destination = oConf.Email
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roTaskCloseToFinish::Translate::", ex)
            oItem = Nothing
        End Try
        Return oItem
    End Function
    ...
    Public Overrides Function GenerateNotificationTasks(ByVal ads As DataRow) As Boolean
        '
        ' Tareas que finalizan hoy
        '
        Dim SQL As String
        Dim bRet As Boolean = True

        Try
            ' Verificamos si existen tareas que finalizan hoy
            ' y aun no tiene alerta creada
            SQL = "@SELECT# Tasks.ID, Tasks.ExpectedEndDate " &
                         "FROM Tasks " &
                         "WHERE Tasks.ExpectedEndDate is not null " &
                                " AND ExpectedEndDate >=  " & roTypes.Any2Time(DateTime.Now.Date).SQLSmallDateTime &
                                " AND ExpectedEndDate <  " & roTypes.Any2Time(DateTime.Now.Date).Add(1, "d").SQLSmallDateTime &
                                " AND Status = 0 " &
                                " AND NOT EXISTS " &
                                "(@SELECT# * " &
            ...
            End If
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roTaskCloseToFinish::GenerateNotificationTasks:: Unexpected error: ", ex)
            bRet = False
        End Try

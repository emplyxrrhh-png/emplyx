...
Public Class roNotificationTask_BreakNotTaken_Manager
    Inherits roNotificationTaskManager

    Public Sub New(ByVal sGUID As String)
        MyBase.New(eNotificationType.BreakNotTaken, sGUID)
    End Sub
    ...
    Protected Overrides Function GetTranslatedNotificationTexts(ByVal oConf As roNotificationDestinationConfig) As roNotificationItem
        ...
        Try
            ...
            oItem.Subject = roNotificationHelper.Message("Notification.BreakNotTaken.Subject", Nothing, , oConf.Language)
            oItem.Body = roNotificationHelper.Message("Notification.BreakNotTaken.Body", Params, , oConf.Language)
            oItem.Content = String.Empty
            oItem.Destination = oConf.Email
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roBreakNotTaken::Translate::", ex)
            oItem = Nothing
        End Try
        Return oItem
    End Function
    ...
    Public Overrides Function GenerateNotificationTasks(ByVal ads As DataRow) As Boolean
        ...
        Try
            ...
            SQL = "@SELECT# distinct(IDShiftUsed), ds.IDEmployee, ds.Date, ds.LayersDefinition, Shifts.StartLimit " &
                        ...
                        "AND p.ActualType = 1 " &
                        "AND p.DateTime >=@Date " &
                        "AND sl.Definition Like '%<Item key=""NotificationForSupervisor"" type=""8"">true</Item>%' ORDER BY Date ASC"
                ...
                For Each drShift As DataRow In dtShifts.Rows 'Por cada shift
                    ...
                    For Each shiftlayer As Shift.roShiftLayer In oShift.Layers
                        ...
                        If shiftlayer.LayerType = roLayerTypes.roLTBreak Then
                            ...
                            Dim notifySupervisor As Boolean = roTypes.Any2Boolean(oData.Item(shiftlayer.XmlKey(roXmlLayerKeys.NotificationForSupervisor)))

                            If notifySupervisor Then
                                ...
                                ' En descanso no realizado no hay tiempo de cortesía
                                Dim courtesyTime As TimeSpan = TimeSpan.Parse("00:00")
                                ...
                                Dim breakPeriodFinished As Boolean = finishRealBreakDateWithTimeZone.Add(courtesyTime) <= nowTime
                                ...
                                ' Sólo notifico si no lo hice antes y si el descanso ya ha finalizado
                                If notificationDoesNotExists AndAlso breakPeriodFinished Then
                                    ...
                                    bRet = roNotificationHelper.CreateBreakNotification(dtPunches, courtesyTime, beginRealBreakDate, finishRealBreakDate, eNotificationType.BreakNotTaken, roTypes.Any2Integer(drShift("IDEmployee")), roTypes.Any2Integer(ads("ID")), beginBreakTimeToNotify, workStartTime)
                                End If
                            End If
                        End If
                    Next
            ...
            End If
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roBreakNotTaken::GenerateNotificationTasks:: Unexpected error: ", ex)
            bRet = False
        End Try

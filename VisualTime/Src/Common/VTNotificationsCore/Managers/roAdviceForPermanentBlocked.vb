...
Public Class roNotificationTask_Advice_For_permanent_blocked_account_Manager
    Inherits roNotificationTaskManager

    Public Sub New(ByVal sGUID As String)
        MyBase.New(eNotificationType.Advice_For_permanent_blocked_account, sGUID)
    End Sub
    ...
    Protected Overrides Function GetTranslatedNotificationTexts(ByVal oConf As roNotificationDestinationConfig) As roNotificationItem
        ...
        Try
            ...
            oItem.Subject = roNotificationHelper.Message("Notification.AdviceBloquedAccount.Subject", Nothing, , oConf.Language)
            oItem.Body = roNotificationHelper.Message("Notification.AdviceBloquedAccount.Body", Params, , oConf.Language)
            oItem.Content = String.Empty
            oItem.Destination = oConf.Email
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roAdviceForPermanentBlocked::Translate::", ex)
            oItem = Nothing
        End Try
        Return oItem
    End Function

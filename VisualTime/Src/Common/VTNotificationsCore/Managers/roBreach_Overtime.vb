...
Public Class roNotificationTask_OvertimeBreach_Manager
    Inherits roNotificationTaskManager

    Public Sub New(ByVal sGUID As String)
        MyBase.New(eNotificationType.OvertimeBreach, sGUID)
    End Sub
    ...
    Protected Overrides Function GetTranslatedNotificationTexts(ByVal oConf As roNotificationDestinationConfig) As roNotificationItem
        ...
        Try
            ...
            Select Case eControlType
                Case eBreachControlType.Both
                    ...
                    strSubject = roNotificationHelper.Message("Notification.OvertimeBreach.Subject", Nothing, , oConf.Language)
                    strBody = roNotificationHelper.Message("Notification.OvertimeBreach.Body", Params, , oConf.Language)
                Case eBreachControlType.OnlyMaximumExceeded
                    Params.Add(roConversions.ConvertMinutesToTime(iMaxMinutes))
                    strSubject = roNotificationHelper.Message("Notification.OvertimeBreach.MaximumExceeded.Subject", Nothing, , oConf.Language)
                    strBody = roNotificationHelper.Message("Notification.OvertimeBreach.MaximumExceeded.Body", Params, , oConf.Language)
                Case eBreachControlType.OnlyMinimumNotReached
                    Params.Add(roConversions.ConvertMinutesToTime(iMinMinutes))
                    strSubject = roNotificationHelper.Message("Notification.OvertimeBreach.MinimumNotReached.Subject", Nothing, , oConf.Language)
                    strBody = roNotificationHelper.Message("Notification.OvertimeBreach.MinimumNotReached.Body", Params, , oConf.Language)
            End Select
        ...
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roBreach_Overtime::Translate::", ex)
            oItem = Nothing
        End Try
        Return oItem
    End Function
    ...
    Public Overrides Function GenerateNotificationTasks(ByVal ads As DataRow) As Boolean
        ...
        Try
            ...
            Dim sTable = "ProgrammedOvertimes"
            sBeginDate = "ProgrammedOvertimes.BeginDate"
            sEndDate = "ProgrammedOvertimes.EndDate"
            ...
            End If
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roBreach_Overtime::GenerateNotificationTasks:: Unexpected error: ", ex)
            bRet = False
        End Try

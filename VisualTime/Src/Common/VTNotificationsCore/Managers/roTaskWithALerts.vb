...
Public Class roNotificationTask_Task_With_ALerts_Manager
    Inherits roNotificationTaskManager

    Public Sub New(ByVal sGUID As String)
        MyBase.New(eNotificationType.Task_With_ALerts, sGUID)
    End Sub
    ...
    Protected Overrides Function GetTranslatedNotificationTexts(ByVal oConf As roNotificationDestinationConfig) As roNotificationItem
        ...
        Try
            ...
            oItem.Subject = roNotificationHelper.Message("Notification.TaskwithAlert.Subject", Nothing, , oConf.Language)
            oItem.Body = roNotificationHelper.Message("Notification.TaskwithAlert.Body", Params, , oConf.Language)
            oItem.Content = String.Empty
            oItem.Destination = oConf.Email
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roTaskWithALerts::Translate::", ex)
            oItem = Nothing
        End Try
        Return oItem
    End Function
    ...
    Public Overrides Function GenerateNotificationTasks(ByVal ads As DataRow) As Boolean
        '
        ' tareas que tienen alertas sin leer
        '
        Dim SQL As String
        Dim bRet As Boolean = True

        Try
            ' Verificamos si existen tareas que tienen alertas sin leer
            ' y aun no tiene alerta creada
            SQL = "@SELECT# IDTask, IDEmployee, DateTime  " &
                         "FROM dbo.AlertsTask " &
                         "WHERE IsReaded =0 " &
                                " AND NOT EXISTS " &
                                "(@SELECT# * " &
                                    " From sysroNotificationTasks " &
                                    " Where sysroNotificationTasks.Key1Numeric = AlertsTask.IDTask" &
                                    " AND AlertsTask.DateTime =  sysroNotificationTasks.Key3Datetime " &
                                    " AND AlertsTask.IDEmployee =  sysroNotificationTasks.Key2Numeric " &
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                For Each dr As DataRow In dt.Rows
                    SQL = "@INSERT# INTO sysroNotificationtasks (IDNotification, Key1Numeric, Key3DateTime, Key2Numeric) VALUES (" &
                              roTypes.Any2Double(ads("ID")) & "," & dr("IDTask") & "," & roTypes.Any2Time(dr("DateTime")).SQLDateTime & "," & dr("IDEmployee") & ")"
                    bRet = bRet AndAlso AccessHelper.ExecuteSql(SQL)
                Next
            End If
        Catch ex As Exception
            roLog.GetInstance().logMessage(roLog.EventType.roError, "roNotificationCore::roTaskWithALerts::GenerateNotificationTasks:: Unexpected error: ", ex)
            bRet = False
        End Try

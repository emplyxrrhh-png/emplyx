...
namespace VisualTimeSageConnector
{
    ...
    public class WSMT
    {
        ...
        private ExternalApi.ExternalApi service = new ExternalApi.ExternalApi();
        ...
        public void ObtenerAccrualsEmpleados(DataTable Empleados)
        {
            try
            {
                ...
                {
                    var accrualCrit = new ExternalApi.roDatalinkStandarAccrualCriteria
                    {
                    ...
                    ExternalApi.roDatalinkStandarAccrualResponse myAccruals = service.GetAccruals(ConfigService.WSCodigoCliente, ConfigService.WSUser, ConfigService.WSPassword, accrualCrit);
            ...
            try
            {
                ...
                foreach (XmlNode Empresa in EmpresasSync)
                {
                    if (Empresa.Attributes.GetNamedItem("id").Value == data["CodigoEmpresa"].ToString())
                    {
                        ...
                        ExternalApi.roDatalinkStandarEmployee Empleado = new ExternalApi.roDatalinkStandarEmployee();
                        ...
                        ExternalApi.roDatalinkEmployeeUserFieldValue myFieldVal;
                        List<ExternalApi.roDatalinkEmployeeUserFieldValue> myTotalFields = new List<ExternalApi.roDatalinkEmployeeUserFieldValue>();
                        ...
                        foreach (XmlNode campoFicha in Empresa.SelectSingleNode("UserFields"))
                        {
                            ...
                            {
                                myFieldVal = new ExternalApi.roDatalinkEmployeeUserFieldValue();
                        ...
                        if (Empleado.IDContract.ToString() != "")
                        {
                            ...
                            service.CreateOrUpdateEmployee(ConfigService.WSCodigoCliente, ConfigService.WSUser, ConfigService.WSPassword, Empleado, out result, out resultSpecified);

                            string errortext = result.ToString();
                            if (Enum.IsDefined(typeof(ReturnCode), System.Convert.ToInt32(result)))
                            {
                                errortext = Enum.Parse(typeof(ReturnCode), result.ToString()).ToString();
                            }
                            ...
                            if (result != 0)
                            {
                                ...
                                notificationbody += "Código de error: " + result.ToString() + "\r\n";
